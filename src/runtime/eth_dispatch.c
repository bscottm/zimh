...
{
#if defined(HAVE_PCAP_NETWORK)
    return pcap_sendpacket((pcap_t *)dev->handle, (u_char *)packet->msg,
                           packet->len);
#else
...
static int eth_writer_dispatch_tap(ETH_DEV *dev, const ETH_PACK *packet)
{
    ...
    return (((int)packet->len ==
             write(dev->fd_handle, (void *)packet->msg, packet->len))
                ? 0
                : -1);
#else
    (void)dev; (void)packet;
...
{
#if defined(HAVE_VDE_NETWORK)
    int status = vde_send((VDECONN*)dev->handle, (void *)packet->msg, packet->len, 0);
...
{
#if defined(HAVE_SLIRP_NETWORK)
    int status = sim_slirp_send((sim_slirp_handle *)dev->handle,
...
{
    return (((int32_t)packet->len == sim_write_sock(dev->fd_handle, (char *)packet->msg, (int32_t)packet->len)) ? 0 : -1);
}
...
const eth_writer_dispatch_fn eth_writer_dispatch_table[ETH_API_COUNT] = {
    [ETH_API_NONE] = eth_writer_dispatch_none,
    [ETH_API_PCAP] = eth_writer_dispatch_pcap,
    [ETH_API_TAP]  = eth_writer_dispatch_tap,
    [ETH_API_VDE]  = eth_writer_dispatch_vde,
    [ETH_API_UDP]  = eth_writer_dispatch_udp,
    [ETH_API_NAT]  = eth_writer_dispatch_nat,
...
{
#if defined(HAVE_PCAP_NETWORK)
    return pcap_dispatch((pcap_t *)dev->handle, -1, &_eth_callback,
                         (u_char *)dev);
#else
    return 0;
#endif
...
const eth_reader_dispatch_fn eth_reader_dispatch_table[ETH_API_COUNT] = {
    [ETH_API_NONE] = eth_reader_dispatch_none,
    [ETH_API_PCAP] = eth_reader_dispatch_pcap,
    [ETH_API_TAP]  = eth_reader_dispatch_tap,
    [ETH_API_VDE]  = eth_reader_dispatch_vde,
    [ETH_API_UDP]  = eth_reader_dispatch_udp,
    [ETH_API_NAT]  = eth_reader_dispatch_nat,
