...
sim_idle_wait = true;
if (sim_cond_timedwait (&sim_asynch_wake, &sim_asynch_lock, &end_time))
    timedout = true;
else
...
if (tmr != SIM_NTIMERS) {
    if (delta_rtime != 0)                           /* avoid divide by zero  */
        last_idle_pct = MIN(100, (uint32_t)(100.0 * (((double)(rtc->clock_time_idled - rtc->clock_time_idled_last)) / ((double)delta_rtime))));
...
if (delta_rtime == 0)                           /* gap too small? */
    ...
    rtc->based = (int32_t) (((double) rtc->based * (double) rtc->nxintv) /
...
rtc->currd = (int32_t) (((double) rtc->based * (double) rtc->nxintv) /
    1000.0);                                        /* next delay */
...
#if defined (SIM_ASYNCH_IO)
sim_mutex_unlock (&sim_timer_lock);
#endif /* SIM_ASYNCH_IO */
return SCPE_OK;
}
...
sim_thread_t        sim_timer_thread;           /* Wall Clock Timing Thread Id */
sim_cond_t      sim_timer_startup_cond;
...
sim_cond_signal (&sim_timer_startup_cond);   /* Signal we're ready to go */
while (sim_asynch_timer && sim_is_running) {
    ...
    if ((wait_usec <= 0.0) ||
        (0 != sim_cond_timedwait (&sim_timer_wake, &sim_timer_lock, &due_time))) {
        ...
        if (1 != sim_timespec_compare (&due_time, &stop_time))
            ...
            inst_delay = (int32_t)(inst_per_sec*(_timespec_to_double(&due_time)-_timespec_to_double(&stop_time)));
...
if (tmr == SIM_NTIMERS) {                   /* None found? */
    if ((tmr != newtmr) && (!sim_is_active (&SIM_INTERNAL_UNIT))) {
        ...
        sim_rtcn_init_unit_ticks (&SIM_INTERNAL_UNIT, (int32_t)((CLK_INIT*CLK_TPS)/sim_int_clk_tps), SIM_INTERNAL_CLK, sim_int_clk_tps);
...
if (sim_asynch_timer && sim_is_running) {
    ...
    sim_cond_init (&sim_timer_startup_cond, NULL);
    ...
    if (create_status != 0) {
        sim_cond_destroy (&sim_timer_startup_cond);
        sim_mutex_unlock (&sim_timer_lock);
        (void) sim_messagef (
            SCPE_IOERR,
    ...
    while (!sim_timer_thread_running)          /* Wait for thread to stabilize */
        sim_cond_wait (&sim_timer_startup_cond, &sim_timer_lock);
    sim_cond_destroy (&sim_timer_startup_cond);
    }
sim_mutex_unlock (&sim_timer_lock);
#endif
...
if (sim_timer_thread_running) {
    ...
    sim_cond_signal (&sim_timer_wake);
    sim_mutex_unlock (&sim_timer_lock);
    sim_thread_join (sim_timer_thread, NULL);
...
if ((sim_asynch_timer) &&
    ...
    if (prvptr == NULL) {                           /* inserting at head */
        ...
        sim_cond_signal (&sim_timer_wake);      /* wake the timer thread to deal with it */
        sim_mutex_unlock (&sim_timer_lock);
        return SCPE_OK;
        }
