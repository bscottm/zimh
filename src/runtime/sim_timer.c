...
/* Convert a timespec to the current host time as a wrapping millisecond
   tick counter.  The uint32_t value intentionally wraps; callers must
...
static uint32_t sim_timer_timespec_msec(const struct timespec *tp)
{
...
uint32_t real_sim_idle_ms_sleep(uint_t msec);
uint32_t real_sim_os_msec(void);
uint32_t real_sim_os_ms_sleep(uint_t msec);
static uint32_t real_sim_os_sleep_min_ms = 0;
static uint32_t real_sim_os_sleep_inc_ms = 0;

uint32_t sim_idle_ms_sleep(uint_t msec)
{
    uint32_t real_start = real_sim_os_msec();
    uint32_t start = (real_start / MS_MIN_GRANULARITY) * MS_MIN_GRANULARITY;
    uint32_t tick_left;
...
bool sim_idle_enab = false;          /* global flag */
volatile bool sim_idle_wait = false; /* global flag */

int32_t sim_vm_initial_ips = SIM_INITIAL_IPS;

static int32_t sim_precalibrate_ips = SIM_INITIAL_IPS;
static int32_t sim_calb_tmr = -1;        /* the system calibrated timer */
static int32_t sim_calb_tmr_last = -1;   /* shadow value when at sim> prompt */
static double sim_inst_per_sec_last = 0; /* shadow value when at sim> prompt */
static uint32_t sim_stop_time = 0;       /* time when sim_stop_timer_services was called */
double sim_time_at_sim_prompt = 0;       /* time spent processing commands from sim> prompt */

static uint32_t sim_idle_rate_ms = 0;    /* Minimum Sleep time */
static uint32_t sim_os_sleep_min_ms = 0;
static uint32_t sim_os_sleep_inc_ms = 0;
static uint32_t sim_os_clock_resoluton_ms = 0;
static uint32_t sim_os_tick_hz = 0;
static uint32_t sim_idle_stable = SIM_IDLE_STDFLT;
static uint32_t sim_idle_calib_pct = 100;
static double sim_timer_stop_time = 0;
static uint32_t sim_rom_delay = 0;
static uint32_t sim_throt_ms_start = 0;
static uint32_t sim_throt_ms_stop = 0;
static uint32_t sim_throt_type = 0;
static uint32_t sim_throt_val = 0;
static uint32_t sim_throt_drift_pct = SIM_THROT_DRIFT_PCT_DFLT;
static uint32_t sim_throt_state = SIM_THROT_STATE_INIT;
static double sim_throt_cps;
static double sim_throt_peak_cps;
static double sim_throt_inst_start;
static uint32_t sim_throt_sleep_time = 0;
static int32_t sim_throt_wait = 0;
static uint32_t sim_throt_delay = 3;
#define CLK_TPS 100
#define CLK_INIT (sim_precalibrate_ips / CLK_TPS)
static int32_t sim_int_clk_tps;
...
static bool sim_catchup_ticks = true;
#if defined(SIM_ASYNCH_CLOCKS) && !defined(SIM_ASYNCH_IO)
#    undef SIM_ASYNCH_CLOCKS
#endif
bool sim_asynch_timer = false;

#if defined(SIM_ASYNCH_CLOCKS)
/* Wallclock timer event structure (non-intrusive) */
...
static sim_wallclock_event_t *volatile sim_wallclock_queue = NULL;
static sim_wallclock_event_t *volatile sim_wallclock_entry = NULL;
#endif

#define sleep1Samples 100

static uint32_t _compute_minimum_sleep(void)
{
    uint32_t i, tot, tim;
...
uint32_t sim_idle_ms_sleep(uint_t msec)
{
    ...
    if (pthread_cond_timedwait(&sim_asynch_wake, &sim_asynch_lock, &end_time))
        ...
        sim_atomic_put(&sim_asynch_check, 0); /* Force check of async queue now */
    sim_idle_wait = false;
    aio_global_unlock();
...
static bool _rtcn_tick_catchup_check(RTC *rtc, int32_t time);
static void _rtcn_configure_calibrated_clock(int32_t newtmr);
static bool _sim_coschedule_cancel(UNIT *uptr);
...
static uint32_t sim_idle_cyc_ms = 0;    /* Cycles per millisecond while not idling */
static uint32_t sim_idle_cyc_sleep = 0; /* Cycles per minimum sleep interval */
...
int32_t sim_internal_timer_time;        /* Pending internal timer delay */
...
void sim_timer_get_throttle_test_state(struct sim_timer_throttle_test_state *state)
{
...
DEBTAB sim_timer_debug[] = {{"TRACE", DBG_TRC, "Trace routine calls"},
...
{
    uint32_t new_rtime, delta_rtime, last_idle_pct, catchup_ticks_curr;
    int32_t delta_vtime;
    double new_gtime;
    int32_t new_currd;
    int32_t itmr;
        ...
        if (ticksper != 0) {
            ...
            if ((tmr != sim_calb_tmr) && rtc->clock_unit && (ticksper > crtc->hz)) {
                ...
                sim_debug(DBG_CAL, &sim_timer_dev,
                          ...
                          sim_uname(rtc->clock_unit), ticksper, sim_os_tick_hz);
    ...
    if (sim_calb_tmr != tmr) {
        ...
        sim_debug(DBG_CAL, &sim_timer_dev,
                  ...
                  sim_calb_tmr, ticksper, rtc->currd);
        return rtc->currd;
    }
    new_rtime = sim_os_msec(); /* wall time */
    if (!sim_signaled_int_char && ((new_rtime - sim_last_poll_kbd_time) > 500)) {
        sim_debug(DBG_CAL, &sim_timer_dev, "sim_rtcn_calb(tmr=%d) gratuitous keyboard poll after %d msecs\n", tmr,
                  (int)(new_rtime - sim_last_poll_kbd_time));
        (void)sim_poll_kbd();
    }
    ...
    if (new_rtime < rtc->rtime) { /* time running backwards? */
        /* This happens when the value returned by sim_os_msec wraps (as a uint32_t) */
    ...
    if (tmr != SIM_NTIMERS) {
        if (delta_rtime != 0)                                       /* avoid divide by zero  */
            last_idle_pct =
                MIN(100, (uint32_t)(100.0 * (((double)(rtc->clock_time_idled - rtc->clock_time_idled_last)) /
                                             ((double)delta_rtime))));
    ...
    if ((last_idle_pct == 0) && (delta_rtime != 0)) {
        sim_idle_cyc_ms = (uint32_t)((new_gtime - rtc->gtime) / delta_rtime);
        if ((sim_idle_rate_ms != 0) && (delta_rtime > 1))
            sim_idle_cyc_sleep = (uint32_t)((new_gtime - rtc->gtime) / (delta_rtime / sim_idle_rate_ms));
    }
    if (sim_asynch_timer || (catchup_ticks_curr > 0)) {
        ...
        new_currd = (int32_t)((new_gtime - rtc->gtime) / ticksper);
    ...
    if (delta_rtime == 0)                                                                           /* gap too small? */
        ...
        rtc->based = (int32_t)(((double)rtc->based * (double)rtc->nxintv) / ((double)delta_rtime)); /* new base rate */
    ...
    rtc->currd = (int32_t)(((double)rtc->based * (double)rtc->nxintv) / 1000.0); /* next delay */
...
{
    int tmr;
    uint32_t clock_start, clock_last, clock_now;
...
t_stat sim_show_timers(FILE *st, DEVICE *dptr, UNIT *uptr, int32_t val, const char *desc)
{
    ...
    struct timespec now;
    time_t time_t_now;
    int32_t calb_tmr = (sim_calb_tmr == -1) ? sim_calb_tmr_last : sim_calb_tmr;
    ...
    for (tmr = clocks = 0; tmr <= SIM_NTIMERS; ++tmr) {
        ...
        if (rtc->clock_unit) {
            ...
            fprintf(st, "%s clock device is %s%s%s\n", sim_name,
                    (tmr == SIM_NTIMERS) ? "Internal Calibrated Timer(" : "", sim_uname(rtc->clock_unit),
                    (tmr == SIM_NTIMERS) ? ")" : "");
        }

        fprintf(st, "%s%sTimer %d:\n", sim_asynch_timer ? "Asynchronous " : "",
                rtc->hz ? "Calibrated " : "Uncalibrated ", tmr);
        if (rtc->hz) {
        ...
        if (rtc->clock_init_base_time) {
            ...
            fprintf(st, "  Initialize Base Time:      %s.%03d\n",
                    sim_timer_format_hms(time_t_now, time_buf, sizeof(time_buf)), (int)(now.tv_nsec / 1000000));
        }
...
t_stat sim_show_clock_queues(FILE *st, DEVICE *dptr, UNIT *uptr, int32_t flag, const char *cptr)
{
    ...
    if (sim_asynch_timer) {
        ...
        if (sim_wallclock_queue == NULL)
            ...
            sim_wallclock_event_t *event;
            fprintf(st, "%s wall clock event queue status\n", sim_name);
            for (event = sim_wallclock_queue; event != NULL; event = event->next) {
                uptr = event->unit;
                ...
                _double_to_timespec(&due, event->a_due_time);
                time_t_due = (time_t)due.tv_sec;
                {
                    char time_buf[16];

                    fprintf(st, " after %s due at %s.%06d\n", tim,
                            sim_timer_format_hms(time_t_due, time_buf, sizeof(time_buf)), (int)(due.tv_nsec / 1000));
                }
            }
        }
    }
...
REG sim_timer_reg[] = {{DRDATAD(IDLE_CYC_MS, sim_idle_cyc_ms, 32, "Cycles Per Millisecond"), PV_RSPC | REG_RO},
...
static t_stat sim_timer_set_catchup(int32_t flag, const char *cptr)
{
...
t_stat sim_timer_show_catchup(FILE *st, UNIT *uptr, int32_t val, const void *desc)
{
...
static t_stat sim_timer_set_idle_pct(int32_t flag, const char *cptr)
{
    /* Generic set command signature.
       This implementation does not use every parameter. */
    (void)flag;
        ...
        return SCPE_ARG;
    {
        ...
        if (isdigit(gbuf[0]))
            newpct = (int32_t)get_uint(gbuf, 10, 100, &r);
        else {
        ...
        if ((r != SCPE_OK) || (newpct == (int32_t)(sim_idle_calib_pct)))
            return r;
        if (newpct == 0)
            return SCPE_ARG;
        sim_idle_calib_pct = (uint32_t)newpct;
    }
    return SCPE_OK;
}
...
static t_stat sim_timer_set_stop(int32_t flag, const char *cptr)
{
    /* Generic set command signature.
       This implementation does not use every parameter. */
    (void)flag;
    ...
    sim_activate_abs(&sim_stop_unit, (int32_t)(sim_timer_stop_time - sim_gtime()));
    return SCPE_OK;
}
...
#if defined(SIM_ASYNCH_CLOCKS)
static t_stat sim_timer_set_async(int32_t flag, const char *cptr)
{
    /* Generic set command signature.
       This implementation does not use every parameter. */
    (void)cptr;
...
DEVICE sim_timer_dev = {"INT-CLOCK",
                        sim_timer_units,
                        sim_timer_reg,
                        sim_timer_mod,
                        SIM_NTIMERS + 1,
                        ...
                        NULL,
                        NULL,
                        DEV_DEBUG | DEV_NOSAVE,
                        0,
                        sim_timer_debug};

DEVICE sim_int_timer_dev = {
    "INT-TIMER", &sim_internal_timer_unit, NULL, NULL, 1, 0, 0, 0, 0, 0, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
    DEV_NOSAVE};

DEVICE sim_stop_dev = {"INT-STOP", &sim_stop_unit,
                       NULL,       NULL,
                       1,          0,
                       0,          0,
                       0,          0,
                       ...
                       NULL,       NULL,
                       NULL,       NULL,
                       NULL,       sim_int_stop_description};

DEVICE sim_throttle_dev = {
    "INT-THROTTLE", &sim_throttle_unit, sim_throttle_reg, NULL, 1, 0, 0, 0, 0, 0, NULL, NULL, NULL, NULL, NULL, NULL,
    NULL,           DEV_NOSAVE};
...
{
    uint32_t w_ms, w_idle, act_ms;
    int32_t act_cyc;
    static bool in_nowait = false;
    ...
    if (rtc->hz == 0) { /* specified timer is not running? */
        ...
        tmr = (uint32_t)sim_calb_tmr; /* use calibrated timer instead */
    }
    rtc = &rtcs[tmr];
    ...
    w_ms = (uint32_t)sim_interval / sim_idle_cyc_ms; /* ms to wait */
    ...
    if (sim_clock_queue == QUEUE_LIST_END)
        ...
        sim_debug(DBG_IDL, &sim_timer_dev, "sleeping for %d ms - pending event on %s in %d %s\n", w_ms,
                  sim_uname(sim_clock_queue), sim_interval, sim_vm_interval_units);
    ...
    if (cyc_since_idle > sim_idle_cyc_sleep)
        ...
        act_cyc -= (int32_t)cyc_since_idle; /* account for cycles executed */
...
t_stat sim_set_throt(int32_t arg, const char *cptr)
{
    ...
    if (arg == 0) {
        ...
        sim_throt_val = (uint32_t)val;
        if (sim_throt_type == SIM_THROT_SPC) {
            if (val2 >= sim_idle_rate_ms)
                sim_throt_sleep_time = (uint32_t)val2;
            else {
                if ((sim_idle_rate_ms % val2) == 0) {
                    sim_throt_sleep_time = sim_idle_rate_ms;
                    sim_throt_val = (uint32_t)(val * (sim_idle_rate_ms / val2));
                } else {
                    sim_throt_sleep_time = sim_idle_rate_ms;
                    sim_throt_val = (uint32_t)(val * (1 + (sim_idle_rate_ms / val2)));
                }
            }
    ...
    if (sim_throt_type == SIM_THROT_SPC) { /* Set initial value while correct one is determined */
        sim_throt_cps = (int32_t)((1000.0 * sim_throt_val) / (double)sim_throt_sleep_time);
...
t_stat sim_throt_svc(UNIT *uptr)
{
    ...
    case SIM_THROT_STATE_INIT: /* take initial reading */
        if ((sim_calb_tmr != -1) && (rtc->hz != 0)) {
            ...
            sim_debug(DBG_THR, &sim_timer_dev,
                      "sim_throt_svc(INIT) Computing Throttling values based on the last second's execution rate\n");
            ...
            sim_debug(DBG_THR, &sim_timer_dev,
                      "sim_throt_svc(INIT) Calibrated timer not available. Falling back to legacy method\n");
        ...
        if (sim_throt_type != SIM_THROT_SPC) { /* dynamic? */
            switch (sim_throt_type) {
            case SIM_THROT_PCT:
                sim_throt_wait = (int32_t)((sim_throt_peak_cps * sim_throt_val) / 100.0);
                break;
            case SIM_THROT_KCYC:
    ...
    case SIM_THROT_STATE_TIME: /* take final reading */
        ...
        if (delta_ms < SIM_THROT_MSMIN) {               /* not enough time? */
            ...
            sim_throt_wait = (int32_t)(delta_inst * SIM_THROT_WMUL);
            sim_throt_inst_start = sim_gtime();
            sim_idle_ms_sleep(sim_idle_rate_ms);  /* start on a tick boundary to calibrate */
            sim_throt_ms_start = sim_os_msec();
        } else {                                  /* long enough */
            ...
            while (1) {
                sim_throt_wait = (int32_t)            /* cycles between sleeps */
                    ((a_cps * d_cps * ((double)sim_throt_sleep_time)) / (1000.0 * (a_cps - d_cps)));
                if (sim_throt_wait >= SIM_THROT_WMIN) /* long enough? */
                    break;
                sim_throt_sleep_time += sim_os_sleep_inc_ms;
                sim_debug(DBG_THR, &sim_timer_dev,
                          "sim_throt_svc() Wait too small, increasing sleep time to %d ms.  Values a_cps = %f, d_cps = "
                          "%f, wait = %d\n",
            ...
            sim_debug(DBG_THR, &sim_timer_dev,
                      "sim_throt_svc() Throttle values a_cps = %f, d_cps = %f, wait = %d, sleep = %d ms\n", a_cps,
                      d_cps, sim_throt_wait, sim_throt_sleep_time);
        ...
        if (delta_ms >= 10000) {   /* recompute every 10 sec */
            ...
            if (sim_throt_type != SIM_THROT_SPC) {    /* when not dynamic throttling */
                ...
                if (fabs(100.0 * (d_cps - a_cps) / d_cps) > (double)sim_throt_drift_pct) {
                    sim_debug(DBG_THR, &sim_timer_dev,
                              "sim_throt_svc() Recalibrating throttle based on values a_cps = %f, d_cps = %f deviating "
                              "by %.2f%% from the desired value\n",
                              a_cps, d_cps, fabs(100.0 * (d_cps - a_cps) / d_cps));
                    if ((a_cps > d_cps) && /* too fast? */
                        ((100.0 * (a_cps - d_cps) / d_cps) > (100 - sim_throt_drift_pct))) {
                        sim_debug(DBG_THR, &sim_timer_dev,
                                  "sim_throt_svc() Restarting calibrating throttle going too fast: a_cps = %f, d_cps = "
                                  "%f deviating by %.2f%% from the desired value\n",
                                  a_cps, d_cps, fabs(100.0 * (d_cps - a_cps) / d_cps));
                        while (1) {
                            sim_throt_wait = (int32_t)            /* cycles between sleeps */
                            ...
                            sim_throt_sleep_time += sim_os_sleep_inc_ms;
                            sim_debug(DBG_THR, &sim_timer_dev,
                                      "sim_throt_svc() Wait too small, increasing sleep time to %d ms.  Values a_cps = "
                                      "%f, d_cps = %f, wait = %d\n",
                                      sim_throt_sleep_time, sim_throt_peak_cps, d_cps, sim_throt_wait);
                        }
                    } else { /* slow or within reasonable range */
                        sim_debug(DBG_THR, &sim_timer_dev,
                                  "sim_throt_svc() Adjusting wait before sleep interval by %d\n",
                                  (int32_t)(((d_cps - a_cps) * (double)sim_throt_wait) / d_cps));
                        sim_throt_wait += (int32_t)(((d_cps - a_cps) * (double)sim_throt_wait) / d_cps);
                    }
                    sim_debug(DBG_THR, &sim_timer_dev,
                              "sim_throt_svc() Throttle values a_cps = %f, d_cps = %f, wait = %d, sleep = %d ms\n",
                ...
                sim_throt_cps = (int32_t)a_cps;
                sim_debug(DBG_THR, &sim_timer_dev,
                          "sim_throt_svc() Recalibrating Special %d/%u Cycles Per Second of %f\n", sim_throt_wait,
                          sim_throt_sleep_time, sim_throt_cps);
                sim_throt_inst_start = sim_gtime();
                sim_throt_ms_start = sim_os_msec();
            }
        }
...
static void _sim_timer_adjust_cal(void)
{
    ...
    for (tmr = 0; tmr <= SIM_NTIMERS; tmr++) {
        ...
        if (rtc->hz) {                                       /* running? */
            rtc->currd = (int32_t)(sim_throt_cps / rtc->hz); /* use throttle calibration */
            rtc->ticks = rtc->hz - 1;                        /* force clock calibration on next tick */
            rtc->rtime = sim_throt_ms_start - 1000 +
                         1000 / rtc->hz; /* adjust calibration parameters to reflect throttled rate */
...
t_stat sim_timer_tick_svc(UNIT *uptr)
{
    int32_t tmr = (int32_t)(uptr - sim_timer_units);
    t_stat stat;
    RTC *rtc = &rtcs[tmr];
            ...
            if ((rtc->hz > 0) && rtc->clock_catchup_eligible) {
                ...
                if (tnow > (rtc->clock_catchup_base_time + (rtc->calib_tick_time + rtc->clock_tick_size))) {
                    if (!rtc->clock_catchup_pending) {
                        sim_debug(
                            DBG_TIK, &sim_timer_dev,
                            "_rtcn_tick_catchup_check(%d) - scheduling catchup tick %d for %s which is behind %s\n",
                            time, 1 + rtc->ticks, sim_uname(rtc->clock_unit),
                            sim_fmt_secs(
                                tnow - (rtc->clock_catchup_base_time + (rtc->calib_tick_time + rtc->clock_tick_size))));
                        rtc->clock_catchup_pending = true;
                        sim_activate_abs(rtc->timer_unit, 0);
                        bReturn = true;
                    } else
                        sim_debug(DBG_TIK, &sim_timer_dev,
                                  "_rtcn_tick_catchup_check(%d) - already pending catchup tick %d for %s which is "
                                  "behind %s\n",
                                  time, 1 + rtc->ticks, sim_uname(rtc->clock_unit),
                                  sim_fmt_secs(tnow - (rtc->clock_catchup_base_time +
                                                       (rtc->calib_tick_time + rtc->clock_tick_size))));
                }
            }
        }
    ...
    if ((rtc->hz > 0) && rtc->clock_catchup_eligible) {
        ...
        if (tnow > (rtc->clock_catchup_base_time + (rtc->calib_tick_time + rtc->clock_tick_size))) {
            if (!rtc->clock_catchup_pending) {
                sim_debug(DBG_TIK, &sim_timer_dev,
                          "_rtcn_tick_catchup_check(%d) - scheduling catchup tick %d for %s which is behind %s\n", time,
                          1 + rtc->ticks, sim_uname(rtc->clock_unit),
                          sim_fmt_secs(tnow -
                                       (rtc->clock_catchup_base_time + (rtc->calib_tick_time + rtc->clock_tick_size))));
                rtc->clock_catchup_pending = true;
                sim_activate_abs(rtc->timer_unit, (time < 0) ? 0 : time);
            } else
                sim_debug(DBG_TIK, &sim_timer_dev,
                          "_rtcn_tick_catchup_check(%d) - already pending catchup tick %d for %s which is behind %s\n",
                          time, 1 + rtc->ticks, sim_uname(rtc->clock_unit),
                          sim_fmt_secs(tnow -
                                       (rtc->clock_catchup_base_time + (rtc->calib_tick_time + rtc->clock_tick_size))));
            return true;
...
t_stat sim_rtcn_tick_ack(uint32_t time, int32_t tmr)
{
    ...
    _rtcn_tick_catchup_check(rtc, (int32_t)time);
...
bool sim_timer_thread_running = false;

static THREAD_FUNC_DEFN(_timer_thread)
{
    ...
    while (sim_asynch_timer && sim_is_running) {
        ...
        double inst_per_sec;
        UNIT *uptr;
        sim_wallclock_event_t *event, *cptr, *prvptr;

        if (sim_wallclock_entry) { /* something to insert in queue? */

            sim_debug(DBG_TIM, &sim_timer_dev, "_timer_thread() - timing %s for %s\n",
                      sim_uname(sim_wallclock_entry->unit),
                      sim_fmt_secs(sim_wallclock_entry->unit->a_usec_delay / 1000000.0));

            event = sim_wallclock_entry;
            sim_wallclock_entry = NULL;

            /* Sorted insertion by due_time */
            prvptr = NULL;
            for (cptr = sim_wallclock_queue; cptr != NULL; cptr = cptr->next) {
                if (event->a_due_time < cptr->a_due_time)
                    break;
                prvptr = cptr;
            }
            if (prvptr == NULL) { /* insert at head */
                event->next = sim_wallclock_queue;
                sim_wallclock_queue = event;
            } else {
                event->next = prvptr->next; /* insert after prvptr */
                prvptr->next = event;
            }
        }
        ...
        if (sim_wallclock_queue == NULL)
            ...
            sim_debug(DBG_TIM, &sim_timer_dev, "_timer_thread() - waiting for %.0f usecs until %.6f for %s\n",
                      wait_usec, sim_wallclock_queue->a_due_time, sim_uname(sim_wallclock_queue->unit));
        if ((wait_usec <= 0.0) || (0 != pthread_cond_timedwait(&sim_timer_wake, &sim_timer_lock, &due_time))) {

            if (sim_wallclock_queue == NULL) /* queue empty? */
                continue;                    /* wait again */
            inst_per_sec = sim_timer_inst_per_sec();

            event = sim_wallclock_queue;
            sim_wallclock_queue = event->next;
            uptr = event->unit;
            ...
            if (1 != sim_timespec_compare(&due_time, &stop_time))
                ...
                inst_delay =
                    (int32_t)(inst_per_sec * (_timespec_to_double(&due_time) - _timespec_to_double(&stop_time)));
            sim_debug(DBG_TIM, &sim_timer_dev, "_timer_thread() - slept %.0fms - activating(%s,%d)\n",
                      1000.0 * (_timespec_to_double(&stop_time) - _timespec_to_double(&start_time)), sim_uname(uptr),
                      inst_delay);
...
static void _rtcn_configure_calibrated_clock(int32_t newtmr)
{
    ...
    for (tmr = 0; tmr < SIM_NTIMERS; tmr++) {
        ...
        if ((rtc->hz) &&                             /* is calibrated AND */
            (rtc->hz <= (uint32_t)sim_os_tick_hz) && /* slower than OS tick rate AND */
        ...
        if ((tmr != newtmr) && (!sim_is_active(&SIM_INTERNAL_UNIT))) {
            if ((sim_calb_tmr != SIM_NTIMERS) &&     /* not internal timer? */
                ...
                if (!crtc->hz) {                     /* now stopped? */
                    sim_debug(DBG_CAL, &sim_timer_dev,
                              "_rtcn_configure_calibrated_clock(newtmr=%d) - Cleaning up stopped timer %s support\n",
            ...
            sim_debug(DBG_CAL | DBG_INT, &sim_timer_dev,
                      "_rtcn_configure_calibrated_clock(newtmr=%d) - Starting Internal Calibrated Timer at %dHz\n",
            ...
            sim_rtcn_init_unit_ticks(&SIM_INTERNAL_UNIT, (int32_t)((CLK_INIT * CLK_TPS) / sim_int_clk_tps),
                                     SIM_INTERNAL_CLK, sim_int_clk_tps);
            SIM_INTERNAL_UNIT.action(&SIM_INTERNAL_UNIT);     /* Force tick to activate timer */
        }
        return;
    }
    if ((tmr == newtmr) && (sim_calb_tmr == newtmr)) /* already set? */
        return;
    if (sim_calb_tmr == SIM_NTIMERS) {               /* was old the internal timer? */
        sim_debug(
            DBG_CAL | DBG_INT, &sim_timer_dev,
            "_rtcn_configure_calibrated_clock(newtmr=%d) - Stopping Internal Calibrated Timer, New Timer = %d (%dHz)\n",
            newtmr, tmr, rtc->hz);
        ...
        if (sim_calb_tmr != -1) {
            ...
            sim_debug(
                DBG_CAL | DBG_INT, &sim_timer_dev,
                "_rtcn_configure_calibrated_clock(newtmr=%d) - Changing Calibrated Timer from %d (%dHz) to %d (%dHz)\n",
...
{
    int32_t tmr;
    uint32_t sim_prompt_time = (sim_gtime() > 0) ? (sim_os_msec() - sim_stop_time) : 0;
    int32_t registered_units = 0;
    ...
    for (tmr = 0; tmr <= SIM_NTIMERS; tmr++) {
        ...
        if (rtc->initd) { /* calibrated clock initialized? */
            ...
            sim_debug(DBG_CAL, &sim_timer_dev,
                      "sim_start_timer_services(tmr=%d) - adjusting calibration real time by %d ms\n", tmr,
                      (int)sim_prompt_time);
            if (rtc->clock_catchup_eligible)
    ...
    if (sim_asynch_timer && sim_is_running) {
        ...
        sim_timer_thread_running = false;
        create_status = sim_thread_create(&sim_timer_thread, _timer_thread, NULL);
        if (create_status != 0) {
            pthread_cond_destroy(&sim_timer_startup_cond);
            pthread_mutex_unlock(&sim_timer_lock);
            (void)sim_messagef(SCPE_IOERR, "INT-CLOCK: can't start asynchronous timer thread: %s\n",
...
void sim_stop_timer_services(void)
{
    ...
    for (tmr = 0; tmr <= SIM_NTIMERS; tmr++) {
        ...
        if (rtc->clock_unit) {
            int32_t clock_time = _sim_activate_time(rtc->timer_unit);
            ...
            if (sim_is_active(rtc->timer_unit)) {
                ...
                sim_debug(DBG_QUE, &sim_timer_dev, "sim_stop_timer_services() - tmr=%d scheduling %s after %d\n", tmr,
                          sim_uname(rtc->clock_unit), clock_time);
            ...
            while (rtc->clock_cosched_queue != QUEUE_LIST_END) {
                ...
                cptr->usecs_remaining =
                    usecs_remaining + floor(1000000.0 * (accum - ((accum > 0) ? 1 : 0)) * rtc->clock_tick_size);
                sim_debug(DBG_QUE, &sim_timer_dev,
                          "sim_stop_timer_services() - tmr=%d scheduling %s after %d and %.0f usecs\n", tmr,
                          sim_uname(cptr), clock_time, cptr->usecs_remaining);
            }
            rtc->cosched_interval = 0;
        }
    }
    ...
    if (sim_timer_thread_running) {
        ...
        while (sim_wallclock_queue != NULL) {
            sim_wallclock_event_t *event = sim_wallclock_queue;
            UNIT *uptr = event->unit;
            double inst_delay_d = uptr->a_due_gtime - sim_gtime();
            int32_t inst_delay;

            sim_wallclock_queue = event->next;
            free(event);  /* Free the event structure */
            ...
            inst_delay = (int32_t)inst_delay_d;
...
t_stat sim_timer_activate_after(UNIT *uptr, double usec_delay)
{
    ...
    if (usec_delay < 0.0) {
        ...
        return sim_messagef(SCPE_IERR, "Negative timer activation delay for %s: %.0f usecs\n", sim_uname(uptr),
                            usec_delay);
    }
    ...
    if (sim_calb_tmr != -1) {
        ...
        if (crtc->hz) {               /* Calibrated Timer available? */
            int32_t inst_til_tick = sim_activate_time(crtc->timer_unit) - 1;
            int32_t ticks_til_calib = crtc->hz - crtc->ticks;
            double usecs_per_tick = floor(1000000.0 / crtc->hz);
            int32_t inst_til_calib = inst_til_tick + ((ticks_til_calib - 1) * crtc->currd);

            if ((uptr != crtc->timer_unit) &&                /* Not scheduling calibrated timer */
                (inst_til_tick > 0)) {                       /* and tick not pending? */
                uint32_t usecs_til_calib = (uint32_t)ceil(inst_til_calib / inst_per_usec);
                ...
                if (inst_delay_d > (double)inst_til_calib) { /* long wait? */
                    ...
                    sim_debug(DBG_TIM, &sim_timer_dev,
                              "sim_timer_activate_after(%s, %.0f usecs) - coscheduling with with calibrated timer(%d), "
                              "ticks=%d, usecs_remaining=%.0f usecs, inst_til_tick=%d, ticks_til_calib=%d, "
                              "usecs_til_calib=%u\n",
                              sim_uname(uptr), usec_delay, sim_calb_tmr, ticks_til_calib, uptr->usecs_remaining,
                              inst_til_tick, ticks_til_calib, usecs_til_calib);
                    sim_debug(DBG_CHK, &sim_timer_dev,
                              "sim_timer_activate_after(%s, %.0f usecs) - result = %.0f usecs, %.0f usecs\n",
                              sim_uname(uptr), usec_delay, sim_timer_activate_time_usecs(ouptr),
                              sim_timer_activate_time_usecs(uptr));
                    return stat;
                }
                if ((usec_delay > (2 * usecs_per_tick)) && (ticks_til_calib > 1)) { /* long wait? */
                    ...
                    sim_debug(DBG_TIM, &sim_timer_dev,
                              "sim_timer_activate_after(%s, %.0f usecs) - coscheduling with with calibrated timer(%d), "
                              "ticks=%d, usecs_remaining=%.0f usecs, inst_til_tick=%d, usecs_til_tick=%.0f\n",
    ...
    if ((sim_asynch_timer) && (usec_delay > sim_idle_rate_ms * 1000.0)) {
        ...
        event = (sim_wallclock_event_t *)malloc(sizeof(sim_wallclock_event_t));
        if (event == NULL)
            return SCPE_MEM;

        event->unit = uptr;
        event->a_due_time = d_now + (usec_delay / 1000000.0);
        event->next = NULL;
        ...
        sim_debug(DBG_TIM, &sim_timer_dev,
                  "sim_timer_activate_after(%s, %.0f usecs) - queueing wallclock addition at %.6f\n", sim_uname(uptr),
                  usec_delay, event->a_due_time);

        pthread_mutex_lock(&sim_timer_lock);
        /* Find insertion point (sorted by due_time) */
        for (cptr = sim_wallclock_queue, prvptr = NULL; cptr != NULL; cptr = cptr->next) {
            if (event->a_due_time < cptr->a_due_time)
                break;
            prvptr = cptr;
        }
            ...
            if (sim_timer_thread_running) {
                while (sim_wallclock_entry) { /* wait for any prior entry has been digested */
                    sim_debug(DBG_TIM, &sim_timer_dev,
                              ...
                              sim_uname(uptr), usec_delay, sim_uname(sim_wallclock_entry->unit));
                    pthread_mutex_unlock(&sim_timer_lock);
                    sim_os_ms_sleep(1);
                    pthread_mutex_lock(&sim_timer_lock);
                }
            }
            sim_wallclock_entry = event;
            pthread_cond_signal(&sim_timer_wake); /* wake the timer thread to deal with it */
            pthread_mutex_unlock(&sim_timer_lock);
            return SCPE_OK;
        } else {                                  /* inserting after prvptr */
            event->next = prvptr->next;
            prvptr->next = event;
            pthread_mutex_unlock(&sim_timer_lock);
            return SCPE_OK;
        }
    ...
    sim_debug(DBG_TIM, &sim_timer_dev,
              "sim_timer_activate_after(%s, %.0f usecs) - queue addition at %d - remnant: %.0f\n", sim_uname(uptr),
              usec_delay, inst_delay, uptr->usecs_remaining);
...
t_stat sim_register_clock_unit_tmr(UNIT *uptr, int32_t tmr)
{
    ...
    rtc->timer_unit->flags = ((tmr == SIM_NTIMERS) ? 0 : UNIT_DIS) | (rtc->clock_unit ? UNIT_IDLE : 0);
    return SCPE_OK;
}
...
{
    int32_t tmr = sim_rtcn_calibrated_tmr();
    int32_t ticks = (interval + (sim_rtcn_tick_size(tmr) / 2)) / sim_rtcn_tick_size(tmr); /* Convert to ticks */
...
t_stat sim_clock_coschedule_tmr(UNIT *uptr, int32_t tmr, int32_t ticks)
{
    ...
    if ((NULL == rtc->clock_unit) || (rtc->hz == 0)) {
        sim_debug(DBG_TIM, &sim_timer_dev,
                  "sim_clock_coschedule_tmr(%s, tmr=%d, ticks=%d) - no clock activating after %d %s\n", sim_uname(uptr),
                  tmr, ticks, ticks * (rtc->currd ? rtc->currd : rtcs[sim_rtcn_calibrated_tmr()].currd),
                  sim_vm_interval_units);
        ...
        if (uptr == rtc->clock_cosched_queue)
            rtc->cosched_interval = rtc->clock_cosched_queue->time;
        sim_debug(DBG_QUE, &sim_timer_dev,
                  "sim_clock_coschedule_tmr(%s, tmr=%d, ticks=%d, hz=%d) - queueing for clock co-schedule, interval "
                  "now: %d\n",
...
static bool _sim_wallclock_cancel(UNIT *uptr)
{
    ...
    /* Check if event is pending in staging area */
    if (sim_wallclock_entry && sim_wallclock_entry->unit == uptr) {
        sim_wallclock_event_t *event = sim_wallclock_entry;
        sim_wallclock_entry = NULL;
        free(event);
        sim_debug(DBG_QUE, &sim_timer_dev, "Canceled Queue Pending Timer Event for %s\n", sim_uname(uptr));
        b_return = true;
    } else {
        /* Search the queue for this unit */
        sim_wallclock_event_t *event, *prev;

        for (event = sim_wallclock_queue, prev = NULL; event != NULL; prev = event, event = event->next) {
            if (event->unit == uptr) {
    ...
    /* Clean up UNIT fields if canceled */
    if (b_return) {
        uptr->a_due_gtime = uptr->a_usec_delay = 0;
        uptr->cancel = NULL;
...
{
    int32_t tmr;
    UNIT *check_unit = (UNIT *)uptr;  /* Cast away const for comparison */
    ...
    for (tmr = 0; tmr <= SIM_NTIMERS; tmr++) {
        ...
        if (rtc->clock_unit == uptr) {
            check_unit = &sim_timer_units[tmr];
            break;
        }
    }

    /* Check staging area */
    if (sim_wallclock_entry && sim_wallclock_entry->unit == check_unit)
        return true;

    /* Search the wallclock queue */
    sim_wallclock_event_t *event;
    for (event = sim_wallclock_queue; event != NULL; event = event->next) {
        if (event->unit == check_unit)
            return true;
    }
...
int32_t _sim_timer_activate_time(UNIT *uptr)
{
    ...
    if (uptr->a_is_active == &_sim_wallclock_is_active) {
        ...
        if (sim_wallclock_entry && sim_wallclock_entry->unit == uptr) {
            ...
            pthread_mutex_unlock(&sim_timer_lock);
            return ((int32_t)d_result) + 1;
        }
        /* Search wallclock queue */
        sim_wallclock_event_t *event;
        for (event = sim_wallclock_queue; event != NULL; event = event->next) {
            if (event->unit == uptr) {
                d_result = uptr->a_due_gtime - sim_gtime();
...
double sim_timer_activate_time_usecs(UNIT *uptr)
{
    ...
    if (uptr->a_is_active == &_sim_wallclock_is_active) {
        ...
        if (sim_wallclock_entry && sim_wallclock_entry->unit == uptr) {
            ...
            sim_debug(DBG_QUE, &sim_timer_dev,
                      "sim_timer_activate_time_usecs(%s) wallclock_entry - %.0f usecs, inst_per_sec=%.0f\n",
        ...
        for (event = sim_wallclock_queue; event != NULL; event = event->next) {
            if (event->unit == uptr) {
                ...
                result = uptr->usecs_remaining + (1000000.0 * (result / sim_timer_inst_per_sec())) + 1;
                sim_debug(DBG_QUE, &sim_timer_dev,
                          "sim_timer_activate_time_usecs(%s) wallclock - %.0f usecs, inst_per_sec=%.0f\n",
                          sim_uname(uptr), result, sim_timer_inst_per_sec());
                return result;
            }
        }
        ...
        for (tmr = 0; tmr <= SIM_NTIMERS; tmr++) {
            ...
            for (cptr = rtc->clock_cosched_queue; cptr != QUEUE_LIST_END; cptr = cptr->next) {
                if (cptr == rtc->clock_cosched_queue) {
                    ...
                    accum += cptr->time;
                if (cptr == uptr) {
                    result = uptr->usecs_remaining +
                             ceil(1000000.0 * ((rtc->currd * accum) + sim_activate_time(&sim_timer_units[tmr]) - 1) /
                                  sim_timer_inst_per_sec());
                    sim_debug(DBG_QUE, &sim_timer_dev,
                              "sim_timer_activate_time_usecs(%s) coscheduled - %.0f usecs, inst_per_sec=%.0f, tmr=%d, "
                              "ticksize=%d, ticks=%d, inst_til_tick=%d, usecs_remaining=%.0f\n",
                              sim_uname(uptr), result, sim_timer_inst_per_sec(), tmr, rtc->currd, accum,
                              sim_activate_time(&sim_timer_units[tmr]) - 1, uptr->usecs_remaining);
                    return result;
                }
            }
        }
    ...
    for (tmr = 0; tmr <= SIM_NTIMERS; tmr++) {
        ...
        if ((uptr == rtc->clock_unit) && (uptr->next)) {
            result = rtc->clock_unit->usecs_remaining +
                     (1000000.0 * (sim_activate_time(&sim_timer_units[tmr]) - 1)) / sim_timer_inst_per_sec();
            sim_debug(DBG_QUE, &sim_timer_dev,
                      "sim_timer_activate_time_usecs(%s) clock - %.0f usecs, inst_per_sec=%.0f, usecs_remaining=%.0f\n",
...
SIM_NOINLINE static uint32_t _rom_swapb(uint32_t val)
{
    return ((val << 24) & 0xff000000u) | ((val << 8) & 0xff0000u) | ((val >> 8) & 0xff00u) | ((val >> 24) & 0xffu);
}

static volatile uint32_t rom_loopval = 0;

SIM_NOINLINE uint32_t sim_rom_read_with_delay(uint32_t val)
{
    uint32_t i, l = sim_rom_delay;
...
SIM_NOINLINE uint32_t sim_get_rom_delay_factor(void)
{
    ...
    if (sim_rom_delay == 0) {
        uint32_t i, ts, te, c = 10000, samples = 0;
        while (1) {
            c = c * 2;
            te = sim_os_msec();
            while (te == (ts = sim_os_msec()))
                ; /* align on ms tick */
...
void sim_timer_precalibrate_execution_rate(void)
{
    ...
    sim_precalibrate_ips = (int32_t)(1000.0 * (sim_precalibrate_ips / (double)(end - start)));

    for (tmr = 0; tmr <= SIM_NTIMERS; tmr++) {
        RTC *rtc = &rtcs[tmr];

        if (rtc->hz)
            rtc->initd = rtc->currd = (int32_t)(((double)sim_precalibrate_ips) / rtc->hz);
    }
    reset_all_p(0);
    sim_run_boot_prep(RU_GO);
    for (tmr = 0; tmr <= SIM_NTIMERS; tmr++) {
