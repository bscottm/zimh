...
size_t sim_disk_pread (SIM_FILE_HANDLE handle, void *buf, size_t bytes, t_offset offset)
{
    ...
    if (!ReadFile(hFile, buf, (DWORD)bytes, &bytesRead, &ovl)) {
        ...
        if (err == ERROR_IO_PENDING) {
            if (!GetOverlappedResult(hFile, &ovl, &bytesRead, TRUE)) {
                err = GetLastError();
                if (err == ERROR_HANDLE_EOF)
                    return 0;
                errno = EIO;
    ...
    result = pread(fd, buf, bytes, (off_t)offset);
...
size_t sim_disk_pwrite (SIM_FILE_HANDLE handle, const void *buf, size_t bytes, t_offset offset)
{
    ...
    if (!WriteFile(hFile, buf, (DWORD)bytes, &bytesWritten, &ovl)) {
        DWORD dwError = GetLastError();
        /* ERROR_IO_PENDING means the operation is asynchronous - wait for it */
        if (dwError == ERROR_IO_PENDING) {
            if (!GetOverlappedResult(hFile, &ovl, &bytesWritten, TRUE)) {
                errno = EIO;
                return (size_t)-1;
            }
        } else {
    ...
    result = pwrite(fd, buf, bytes, (off_t)offset);
 ...
 */
SIM_FILE_HANDLE sim_disk_open_handle (const char *file)
{
#if defined(_WIN32)
    HANDLE h;
 ...
 */
SIM_FILE_HANDLE sim_fopen_handle (const char *file, const char *mode)
{
#if defined(_WIN32)
...
t_offset sim_fsize_handle (SIM_FILE_HANDLE handle)
{
#if defined(_WIN32)
    LARGE_INTEGER size;
    if (handle == SIM_INVALID_HANDLE)
