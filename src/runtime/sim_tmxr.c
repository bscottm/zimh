...
// SPDX-FileCopyrightText: 2001-2011 Robert M Supnik
...
typedef struct framer_data {
    ...
    bool connect_pending;           /* True if connected not yet reported */
} FRAMER;
...
#define TMXR_GUARD  ((int32_t)(lp->serport ? 1 : sizeof(mantra)))/* buffer guard */
...
/* Preserve the existing sim_master_sock macro semantics behind a hookable
   function pointer. */
static SOCKET tmxr_default_master_sock (char *port, t_stat *status)
{
return sim_master_sock (port, status);
}

static tmxr_master_sock_fn tmxr_master_sock_hook = tmxr_default_master_sock;
static tmxr_accept_conn_ex_fn tmxr_accept_conn_ex_hook = sim_accept_conn_ex;
static tmxr_connect_sock_ex_fn tmxr_connect_sock_ex_hook = sim_connect_sock_ex;
static tmxr_check_conn_fn tmxr_check_conn_hook = sim_check_conn;
static tmxr_close_sock_fn tmxr_close_sock_hook = sim_close_sock;
...
/* Restore the default host-I/O entry points after tests override them. */
void tmxr_reset_io_hooks (void)
{
tmxr_master_sock_hook = tmxr_default_master_sock;
tmxr_accept_conn_ex_hook = sim_accept_conn_ex;
tmxr_connect_sock_ex_hook = sim_connect_sock_ex;
tmxr_check_conn_hook = sim_check_conn;
tmxr_close_sock_hook = sim_close_sock;
...
/* Tests can override the small set of host-I/O entry points used by the
   TMXR reset and reconnect paths. */
...
tmxr_master_sock_hook = hooks->master_sock ?
    hooks->master_sock : tmxr_default_master_sock;
tmxr_accept_conn_ex_hook = hooks->accept_conn_ex ?
    hooks->accept_conn_ex : sim_accept_conn_ex;
tmxr_connect_sock_ex_hook = hooks->connect_sock_ex ?
    hooks->connect_sock_ex : sim_connect_sock_ex;
tmxr_check_conn_hook = hooks->check_conn ?
    hooks->check_conn : sim_check_conn;
tmxr_close_sock_hook = hooks->close_sock ?
    hooks->close_sock : sim_close_sock;
tmxr_write_sock_hook = hooks->write_sock ?
    hooks->write_sock : sim_write_sock;
tmxr_getnames_sock_hook = hooks->getnames_sock ?
...
tmxr_ms_sleep_hook = hooks->ms_sleep ?
    hooks->ms_sleep : sim_os_ms_sleep;
tmxr_open_serial_hook = hooks->open_serial ?
    hooks->open_serial : sim_open_serial;
tmxr_eth_devices_hook = hooks->eth_devices ?
...
/* Replace the contents of the TMXR-owned string field at *dest with a
   ...
   The destination pointer is assumed to either be NULL or point to storage
   already owned by TMXR.  The helper resizes that storage to fit the new
   string exactly, copies the replacement text including the terminating NUL,
   and leaves ownership with the caller via the updated pointer.
*/
static void tmxr_replace_string (char **dest, const char *src)
{
...
{
int32_t unwritten, psave;
char *msgbuf = tmxr_connection_message (mp, lp,
                                        (sim_switches & SWMASK ('V')) != 0);
const char *message = msgbuf ? msgbuf : "";

if (!mp->buffered) {
    lp->txbpi = 0;                                      /* init buf pointers */
    lp->txbpr = (int32_t)(lp->txbsz - strlen (message));
...
tmxr_linemsg (lp, message);                             /* beginning of buffer */
...
lp->txcnt -= (int32_t)strlen (message);                 /* adjust statistics */
free (msgbuf);
}
...
static int32_t loop_write_ex (TMLN *lp, char *buf, int32_t length, bool prefix_datagram)
{
int32_t written = 0;
int32_t loopfree = lp->lpbsz - lp->lpbcnt;
...
static int32_t loop_write (TMLN *lp, char *buf, int32_t length)
{
return loop_write_ex (lp, buf, length, true);
}

static int32_t loop_read_ex (TMLN *lp, char *buf, int32_t bufsize)
{
int32_t bytesread = 0;

while (bufsize > 0) {
    int32_t chunksize;
    int32_t loopused = lp->lpbcnt;
...
static int32_t loop_read (TMLN *lp, char *buf, int32_t bufsize)
{
if (lp->datagram) {
    int32_t pktsize;

    if (lp->lpbcnt < (int32_t)sizeof(pktsize))
        return 0;
...
static int32_t tmxr_read (TMLN *lp, int32_t length)
{
int32_t i = lp->rxbpi;
...
static int32_t tmxr_write (TMLN *lp, int32_t length)
{
int32_t written = 0;
int32_t i = lp->txbpr;
...
if (lp->serport) {                                      /* serial port connection? */
    ...
    if (lp->framer)
        ...
        if (lp->sock) {                                     /* Telnet connection */
            written = tmxr_write_sock_hook (lp->sock, &(lp->txb[i]), length);
...
static void tmxr_rmvrc (TMLN *lp, int32_t p)
{
...
static TMLN *tmxr_find_ldsc (UNIT *uptr, int32_t val, const TMXR *mp)
{
...
if (uptr) {                                             /* called from SET? */
    ...
    if (dptr == NULL)                                   /* what?? */
        return NULL;
    val = (int32_t) (uptr - dptr->units);               /* implicit line # */
    }
...
if (mp == NULL)                                         /* missing mux descriptor? */
    ...
    if (code == SCPE_OK)                                /* line number OK? */
        lp = mp->ldsc + (int32_t) ln;                   /* use as index to determine line */
    }
...
int32_t tmxr_poll_conn (TMXR *mp)
{
SOCKET newsock;
TMLN *lp;
int32_t *op;
...
if (mp->master) {
    if (mp->ring_sock != INVALID_SOCKET) {  /* Use currently 'ringing' socket if one is active */
        ...
        newsock = tmxr_accept_conn_ex_hook (
            mp->master, &address,
            (mp->packet ? SIM_SOCK_OPT_NODELAY : 0));   /* poll connect */
    ...
    if (newsock != INVALID_SOCKET) {                    /* got a live one? */
        ...
        if (newsock != INVALID_SOCKET) {
            ...
            if (i >= mp->lines) {                           /* all busy? */
                ...
                if (!lp->notelnet) {
                    tmxr_write_sock_hook (newsock, (char *)mantra,
                                          sizeof (mantra));
        ...
        switch ((j+r)&1) {
            case 0:
                if (lp->connecting) {                           /* connecting? */
                    ...
                    switch (tmxr_check_conn_hook (lp->connecting, false))
                        {
                        case 1:                                 /* successful connection */
                            ...
                            lp->cnms = sim_os_msec ();
                            tmxr_getnames_sock_hook (lp->sock, &sockname,
                                                     &peername);
            ...
            case 1:
                if (lp->master) {                                   /* Check for a pending Telnet/tcp connection */
                    while (INVALID_SOCKET !=
                        ...
                        tmxr_getnames_sock_hook (newsock, &sockname,
                                                 &peername);
                        ...
                        if (lp->destination) {                      /* Virtual Null Modem Cable? */
                            ...
                            if (lp->connecting) {
                                ...
                                tmxr_debug_connect_line (lp, msg);
                                tmxr_close_sock_hook (lp->connecting);/* abort our as yet unconnected socket */
                                lp->connecting = 0;
                                }
                            }
                        if (lp->conn == false) {                    /* is the line available? */
...
static t_stat tmxr_reset_ln_ex (TMLN *lp, bool closeserial)
{
char msg[512];
...
if (lp->serport) {
    if (closeserial) {
        tmxr_close_serial_hook (lp->serport);
        lp->serport = 0;
        lp->ser_connect_pending = false;
        free (lp->destination);
        lp->destination = NULL;
        ...
        if (!lp->modem_control) {                       /* serial connection? */
            tmxr_control_serial_hook (lp->serport, 0,
                                      TMXR_MDM_DTR|TMXR_MDM_RTS, NULL);
            tmxr_ms_sleep_hook (TMXR_DTR_DROP_TIME);
            tmxr_control_serial_hook (lp->serport,
                                      TMXR_MDM_DTR|TMXR_MDM_RTS, 0, NULL);
            }
    }
else                                                    /* Telnet connection */
    if (lp->sock) {
        tmxr_close_sock_hook (lp->sock);                /* close socket */
...
if ((lp->destination) && (!lp->serport)) {
    if (lp->connecting) {
        tmxr_close_sock_hook (lp->connecting);
        lp->connecting = 0;
        }
    if ((!lp->modem_control) || (lp->modembits & TMXR_MDM_DTR)) {
        snprintf (msg, sizeof (msg),
                  "tmxr_reset_ln_ex() - connecting to %s",
                  lp->destination);
        tmxr_debug_connect_line (lp, msg);
        lp->connecting = tmxr_connect_sock_ex_hook (
            lp->datagram ? lp->port : NULL, lp->destination, "localhost",
            NULL, (lp->datagram ? SIM_SOCK_OPT_DATAGRAM : 0) |
...
*/
static t_stat tmxr_clear_modem_control_passthru_state (TMXR *mp, bool state)
{
int i;
...
*/
static t_stat tmxr_set_notelnet_state (TMXR *mp, bool state)
{
int i;
...
/* Declare that all lines on a mux will or won't spit out a message
   ...
   ports on the MUX are attached.
*/
static t_stat tmxr_set_nomessage_state (TMXR *mp, bool state)
{
int i;
...
*/
t_stat tmxr_set_get_modem_bits (TMLN *lp, int32_t bits_to_set, int32_t bits_to_clear, int32_t *status_bits)
{
int32_t before_modem_bits, incoming_state;
DEVICE *dptr;
...
if ((lp->sock) || (lp->serport) || (lp->loopback)) {
    ...
    if (((before_modem_bits & TMXR_MDM_DTR) == 0) &&    /* Upward transition of DTR? */
        ...
        if ((lp->destination == NULL) &&
            ...
            if (!lp->notelnet) {
                tmxr_write_sock_hook (lp->sock, (char *)mantra,
                                      sizeof (mantra));
                tmxr_debug (TMXR_DBG_XMT, lp, "Sending", (char *)mantra, sizeof(mantra));
                lp->telnet_sent_opts = (uint8_t *)realloc (lp->telnet_sent_opts, 256);
                memset (lp->telnet_sent_opts, 0, 256);
...
if (lp->mp && lp->modem_control) {                  /* This API ONLY works on modem_control enabled multiplexer lines */
    if ((bits_to_set | bits_to_clear) || status_bits) {/* Anything to do? */
        ...
        if (lp->serport) {
            t_stat r = tmxr_control_serial_hook (lp->serport, bits_to_set,
                                                 bits_to_clear, status_bits);
        ...
        if ((lp->sock) || (lp->connecting)) {
            ...
            if ((lp->destination) &&                    /* Virtual Null Modem Cable */
                ...
                snprintf (msg, sizeof (msg),
                          "tmxr_set_get_modem_bits() - establishing outgoing "
                          "connection to: %s", lp->destination);
                tmxr_debug_connect_line (lp, msg);
                lp->connecting = tmxr_connect_sock_ex_hook (
                    lp->datagram ? lp->port : NULL, lp->destination,
                    "localhost", NULL,
...
if ((lp->serport) && (!lp->loopback))
    tmxr_control_serial_hook (lp->serport, 0, 0, status_bits);
return SCPE_INCOMP;
}
...
*/
t_stat tmxr_set_line_halfduplex (TMLN *lp, bool enable_halfduplex)
{
DEVICE *dptr = (lp->dptr ? lp->dptr : (lp->mp ? lp->mp->dptr : NULL));
...
if (lp->serport) {
    ...
    config_size = strlen (config) + 1;
    lp->serconfig = (char *)realloc (lp->serconfig, config_size);
    strlcpy (lp->serconfig, config, config_size);
...
static int32_t tmxr_rqln_bare (const TMLN *lp, bool speed)
{
if (speed) {
...
int32_t tmxr_rqln (const TMLN *lp)
{
return tmxr_rqln_bare (lp, true);
}

int32_t tmxr_input_pending_ln (TMLN *lp)
{
...
t_stat tmxr_putc_ln (TMLN *lp, int32_t chr)
{
...
int32_t tmxr_send_buffered_data (TMLN *lp)
{
int32_t nbytes, sbytes;
t_stat r;
...
int32_t tmxr_tqln (const TMLN *lp)
{
...
int32_t tmxr_tpqln (const TMLN *lp)
{
...
int32_t tmxr_txdone_ln (TMLN *lp)
{
if (lp->txdone)
...
static void _mux_detach_line (TMLN *lp, bool close_listener, bool close_connecting)
{
if (lp->framer) {
...
if (lp->serport) {                          /* close current serial connection */
    tmxr_reset_ln (lp);
    tmxr_control_serial_hook (lp->serport, 0, TMXR_MDM_DTR|TMXR_MDM_RTS,
                              NULL);
    tmxr_close_serial_hook (lp->serport);
    lp->serport = 0;
    free (lp->serconfig);
...
static t_stat tmxr_parse_framer_spec (const char *framer, char *fr_eth,
                                      size_t fr_eth_size, int8_t *fr_mode,
                                      int32_t *fr_speed)
{
const char *cptr;
char option[CBUFSIZE];
int32_t speed;
bool fr_eth_truncated;
...
static int32_t _tmln_speed_delta_bps (int32_t nspeed)
{
static struct {
    int32_t bps;
    int32_t delta;
...
static int32_t _tmln_speed_delta (const char *cptr)
{
int32_t nspeed;
int nfactor = 1;
...
if (*cptr == '*') {
    ...
    if (!(lp->serport) &&               /* Not a serial port */
        ...
        if ((!uptr) && (lp->mp))
            uptr = lp->mp->uptr;
        lp->rxdeltausecs = (uint32_t)(_tmln_speed_delta_bps ((int32_t)lp->rxbps) /
                                    lp->bpsfactor);
...
static const char* _tmxr_getname (int number, char *name, size_t name_size)
{
  ETH_LIST  list[ETH_MAX_DEVICE];
  int count = tmxr_eth_devices_hook (ETH_MAX_DEVICE, list, true);
...
while (*tptr) {
    ...
    packet = mp->packet;
    if (mp->buffered)
        snprintf (buffered, sizeof (buffered), "%d", mp->buffered);
    if (line != -1) {
        notelnet = listennotelnet = mp->notelnet;
        nomessage = listennomessage = mp->nomessage;
        }
    ...
    while (*tptr) {
        ...
        if (!isdigit(*cptr)) {
            ...
            if (0 == MATCH_CMD (gbuf, "LINE")) {
                ...
                nextline = (int32_t) get_uint (cptr, 10, mp->lines-1, &r);
                if (r)
           ...
           if ((0 == MATCH_CMD (gbuf, "NOBUFFERED")) ||
            ...
            if (0 == MATCH_CMD (gbuf, "BUFFERED")) {
                if ((NULL == cptr) || ('\0' == *cptr))
                    strlcpy (buffered, "32768", sizeof (buffered));
                else {
                    i = (int32_t) get_uint (cptr, 10, 1024*1024, &r);
                    if (r || (i == 0))
                        return sim_messagef (SCPE_ARG, "Invalid Buffered Specifier: %s\n", cptr);
                    snprintf (buffered, sizeof (buffered), "%d", i);
                    }
                continue;
                }
        ...
        sock = tmxr_master_sock_hook (port, &r);                /* make master socket to validate port */
        if (r)
            return sim_messagef (SCPE_ARG, "Invalid Port Specifier: %s\n", port);
    ...
    if (destination[0]) {
        ...
        serport = tmxr_open_serial_hook (destination, NULL, &r);
        if (serport != INVALID_HANDLE) {
            tmxr_close_serial_hook (serport);
    ...
    if (line == -1) {
        ...
        if ((listen[0]) && (!datagram)) {
            sock = tmxr_master_sock_hook (listen, &r);      /* make master socket */
            if (r)
            ...
            if (mp->port) {                                 /* close prior listener */
                tmxr_close_sock_hook (mp->master);
                mp->master = 0;
                free (mp->port);
                mp->port = NULL;
                }
            sim_messagef (SCPE_OK, "Listening on port %s\n", listen);
            tmxr_replace_string (&mp->port, listen);       /* save port */
        ...
        if (framer[0]) {
            ...
            if ((strlen(fr_eth) == 5 || strlen(fr_eth) == 6)
                ...
                num = atoi(&fr_eth[4]);
                if (_tmxr_getname (num, fr_eth, sizeof (fr_eth)) == NULL)
                    return SCPE_OPENERR;
            }
            ...
            r = tmxr_eth_open_hook (eth, fr_eth, mp->dptr, 0);
            if (r != SCPE_OK) {
            ...
            r = tmxr_eth_filter_hook (eth, 1, &(eth->host_nic_phy_hw_addr),
                                      0, 0);
            if (r != SCPE_OK) {
                sim_messagef (r, "Eth set address filter error %d\n", r);
                tmxr_eth_close_hook (eth);
                free (eth);
                free (framer_s);
                return r;
                }
        ...
        if (logfiletmpl[0]) {
            sim_close_logfile (&lp->txlogref);
            lp->txlog = NULL;
            tmxr_replace_string (&lp->txlogname, logfiletmpl);
            r = sim_open_logfile (lp->txlogname, true, &lp->txlog, &lp->txlogref);
            if (r == SCPE_OK)
        ...
        if ((listen[0]) && (!datagram)) {
            ...
            tmxr_replace_string (&lp->port, listen);        /* save port */
...
t_stat tmxr_show_open_devices (FILE* st, DEVICE *dptr, UNIT* uptr, int32_t val, const char* cptr)
{
int i;
char gbuf[CBUFSIZE];
...
for (i = 0; i < mp->lines; i++) {  /* loop thru conn */
    ...
    if (lp->master) {
        tmxr_close_sock_hook (lp->master);              /* close master socket */
        lp->master = 0;
        free (lp->port);
        lp->port = NULL;
        }
...
if (mp->master)
    tmxr_close_sock_hook (mp->master);                  /* close master socket */
mp->master = 0;
free (mp->port);
mp->port = NULL;
if (mp->ring_sock != INVALID_SOCKET) {
    tmxr_close_sock_hook (mp->ring_sock);
    mp->ring_sock = INVALID_SOCKET;
    free (mp->ring_ipad);
    mp->ring_ipad = NULL;
    mp->ring_start_time = 0;
...
for (i=0; i < mp->lines; i++) {
    ...
    snprintf (portname, sizeof (portname), "%s:%d", mp->dptr->name, i);
...
static int32_t _tmxr_activate_delay (UNIT *uptr, int32_t interval)
{
TMXR *mp = (TMXR *)uptr->tmxr;
int32_t i, sooner = interval, due;
double sim_gtime_now = sim_gtime ();
...
for (i=0; i<mp->lines; i++) {
    ...
    if ((lp->conn || lp->txbfd) &&      /* Connected (or buffered)? */
        ...
        if (lp->txnexttime > sim_gtime_now)
            due = (int32_t)(lp->txnexttime - sim_gtime_now);
        else {
...
t_stat tmxr_activate (UNIT *uptr, int32_t interval)
{
int32_t sooner;
...
t_stat tmxr_clock_coschedule (UNIT *uptr, int32_t interval)
{
...
t_stat tmxr_clock_coschedule_abs (UNIT *uptr, int32_t interval)
{
sim_cancel (uptr);
return tmxr_clock_coschedule (uptr, interval);
}

t_stat tmxr_clock_coschedule_tmr (UNIT *uptr, int32_t tmr, int32_t ticks)
{
int32_t interval = ticks * sim_rtcn_tick_size (tmr);
int32_t sooner;
...
t_stat tmxr_clock_coschedule_tmr_abs (UNIT *uptr, int32_t tmr, int32_t ticks)
{
sim_cancel (uptr);
return tmxr_clock_coschedule_tmr (uptr, tmr, ticks);
}
...
t_stat tmxr_attach_help(FILE *st, DEVICE *dptr, UNIT *uptr, int32_t flag, const char *cptr)
{
TMXR *mux = (TMXR *)dptr->help_ctx;
bool single_line = false;                 /* default to Multi-Line help */
bool port_speed_control = false;
bool modem_control = false;
...
if ((sock) && (sock != INVALID_SOCKET))
    tmxr_write_sock_hook (sock, msg, (int32_t)strlen (msg));
}
...
{
while (*msg) {
    while (SCPE_STALL == tmxr_putc_ln (lp, (int32_t)(*msg)))
...
void tmxr_fconns (FILE *st, const TMLN *lp, int32_t ln)
{
int32_t hr, mn, sc;
uint32_t ctime;
...
if (lp->sock) {
    ...
    tmxr_getnames_sock_hook (lp->sock, &sockname, &peername);
...
void tmxr_fstats (FILE *st, const TMLN *lp, int32_t ln)
{
...
t_stat tmxr_dscln (UNIT *uptr, int32_t val, const char *cptr, void *desc)
{
TMXR *mp = (TMXR *) desc;
TMLN *lp;
t_stat status;
...
t_stat tmxr_set_log (UNIT *uptr, int32_t val, const char *cptr, void *desc)
{
TMXR *mp = (TMXR *) desc;
TMLN *lp;
t_stat r;
...
t_stat tmxr_set_nolog (UNIT *uptr, int32_t val, const char *cptr, void *desc)
{
TMXR *mp = (TMXR *) desc;
TMLN *lp;
...
t_stat tmxr_show_log (FILE *st, UNIT *uptr, int32_t val, const void *desc)
{
...
t_stat tmxr_set_lnorder (UNIT *uptr, int32_t val, const char *cptr, void *desc)
{
TMXR   *mp = (TMXR *) desc;
char   *tbuf = NULL;
char   *tptr;
...
list = (int32_t *) calloc (mp->lines, sizeof (int32_t)); /* allocate the new line order array */

if (list == NULL)                                       /* if the allocation failed */
    return SCPE_MEM;                                    /*   then report a "Memory exhausted" error */

set = (bool *) calloc (mp->lines, sizeof (bool));       /* allocate the line set tracking array */
...
t_stat tmxr_show_lnorder (FILE *st, UNIT *uptr, int32_t val, const void *desc)
{
int32_t i, j, low, last;
const TMXR *mp = (const TMXR *) desc;
int32_t *iptr = mp->lnorder;
bool first = true;
...
t_stat tmxr_show_summ (FILE *st, UNIT *uptr, int32_t val, const void *desc)
{
const TMXR *mp = (const TMXR *) desc;
int32_t i, t;
...
t_stat tmxr_show_cstat (FILE *st, UNIT *uptr, int32_t val, const void *desc)
{
const TMXR *mp = (const TMXR *) desc;
int32_t i, any;
...
t_stat tmxr_show_sync_devices (FILE* st, DEVICE *dptr, UNIT* uptr, int32_t val, const char *desc)
{
/* Generic callback signature.
   This implementation does not use every parameter. */
(void)dptr;
...
t_stat tmxr_show_lines (FILE *st, UNIT *uptr, int32_t val, const void *desc)
{
const TMXR *mp = (const TMXR *) desc;
...
{
uint32_t i;
int32_t *saved_lnorder = tmxr->lnorder;
int32_t saved_lines = tmxr->lines;

tmxr->lnorder = (int32_t *)calloc (tmxr->lines, sizeof (*tmxr->lnorder));
if (tmxr->lines >= 8) {
    tmxr->lines = 8;
