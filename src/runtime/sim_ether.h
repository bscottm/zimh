...
#if (!defined(xBSD) && !defined(_WIN32)) || defined(HAVE_TAP_NETWORK) ||       \
...
#define ETH_MAX_JUMBO_FRAME                                                    \
    65536 /* maximum ethernet jumbo frame size (or Offload Segment Size) */
#define ETH_MAX_DEVICE 40 /* maximum ethernet devices */
#define ETH_CRC_SIZE 4    /* ethernet CRC size */
#define ETH_FRAME_SIZE                                                         \
    (ETH_MAX_PACKET + ETH_CRC_SIZE) /* ethernet maximum frame size */
#define ETH_MIN_JUMBO_FRAME                                                    \
    ETH_MAX_PACKET /* Threshold size for Jumbo Frame Processing */
...
#define LOOPBACK_PHYSICAL_RESPONSE(dev, msg)                                   \
    ((dev->have_host_nic_phy_addr) && ((msg)[12] == 0x90) &&                   \
     ...
     ((msg)[16] == 0x02) &&                                                    \
     ((msg)[17] == 0x00) && /* Last Function - Forward */                      \
...
#endif
    /* sim_slirp_handle* goes here but we can't reference it without including
     * sim_slirp.h, which would create circular dependency. Legacy void* handle
     * field used for SLiRP. */
...
struct eth_device {
    ...
    eth_api_t
        eth_api; /* Designator for which API is being used to move packets */
...
typedef eth_reader_state_t (*eth_reader_state_handler_t)(
    eth_reader_context_t *ctx);
typedef eth_writer_state_t (*eth_writer_state_handler_t)(
