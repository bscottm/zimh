...
#    define LOOPBACK_PHYSICAL_RESPONSE(dev, msg)                                                                       \
        ((dev->have_host_nic_phy_addr) && ((msg)[12] == 0x90) && ((msg)[13] == 0x00) && /* Ethernet Loopback */        \
...
#    define LOOPBACK_REFLECTION_TEST_PACKET(dev, msg)                                                                  \
        ((dev->have_host_nic_phy_addr) && ((msg)[12] == 0x90) && ((msg)[13] == 0x00) && /* Ethernet Loopback */        \
         ...
         ((msg)[02] == 0xFF) && ((msg)[03] == 0xFF) && ((msg)[04] == 0xFF) && ((msg)[05] == 0xFE) &&                   \
         (memcmp(dev->host_nic_phy_hw_addr, (msg) + 6, 6) == 0)) /* Ethernet Source - Host MAC */

struct eth_device {
    char *name;                                                  /* Ethernet device name */
    eth_backend_t *backend;                                      /* Discriminated union of API-specific data */
...
void eth_packet_trace_ex(ETH_DEV *dev, const uint8_t *msg, int len, const char *txt, int detail,
...
void eth_mac_fmt(const ETH_MAC add, char *buffer, size_t buffer_size); /* format ethernet mac address */
...
void ethq_item_free(sim_tailq_item_t item);       /* Cleanup item's allocated internals */
...
/* Ethernet group MAC address predicate. Group MAC addresses are also referred to as "multicast". */
...
{
    return is_eth_groupmac(data) && eth_mac_equal(data, eth_mac_bcast);
}
