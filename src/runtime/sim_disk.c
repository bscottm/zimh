...
static VHDHANDLE sim_vhd_disk_open (const char *rawdevicename, const char *openmode);
static VHDHANDLE sim_vhd_disk_create (const char *szVHDPath, t_offset desiredsize);
static VHDHANDLE sim_vhd_disk_create_diff (const char *szVHDPath, const char *szParentVHDPath);
static VHDHANDLE sim_vhd_disk_merge (const char *szVHDPath, char **ParentVHD);
static int sim_vhd_disk_close (VHDHANDLE hVHD);
static void sim_vhd_disk_flush (VHDHANDLE hVHD);
static t_offset sim_vhd_disk_size (VHDHANDLE hVHD);
...
static t_stat sim_vhd_disk_set_dtype (VHDHANDLE hVHD, const char *dtype, uint32_t SectorSize, uint32_t xfer_element_size);
static const char *sim_vhd_disk_get_dtype (VHDHANDLE hVHD, uint32_t *SectorSize, uint32_t *xfer_element_size, char sim_name[64], time_t *creation_time);
static t_stat sim_os_disk_implemented_raw (void);
static sim_raw_osfile_t sim_os_disk_open_raw (const char *rawdevicename, const char *openmode);
static int sim_os_disk_close_raw (sim_raw_osfile_t f);
static void sim_os_disk_flush_raw (sim_raw_osfile_t f);
static t_offset sim_os_disk_size_raw (sim_raw_osfile_t f);
static t_stat sim_os_disk_unload_raw (sim_raw_osfile_t f);
static bool sim_os_disk_isavailable_raw (sim_raw_osfile_t f);
...
static t_stat sim_os_disk_info_raw (sim_raw_osfile_t f, uint32_t *sector_size, uint32_t *removable, uint32_t *is_cdrom);
...
/* sim_disk_image_t initialization functions: */
static inline void sim_file_handle_init(sim_disk_image_t *fd)
{
    fd->native_fd = SIM_INVALID_OSFILE;
}

static inline void sim_vhd_opaque_init(sim_disk_image_t *fd)
{
    fd->opaque_ptr = NULL;
}


/* Wrapper functions to adapt VHD functions to SIM_FILE_HANDLE signature */
static int sim_vhd_disk_close_wrapper (sim_disk_image_t f)
{
    return sim_vhd_disk_close(sim_disk_get_vhd_handle(f));
}

static t_offset sim_vhd_disk_size_wrapper (sim_disk_image_t f)
{
    return sim_vhd_disk_size(sim_disk_get_vhd_handle(f));
}

struct sim_disk_fmt {
    const char          *name;                          /* name */
    int32_t             uflags;                         /* unit flags */
    int32_t             fmtval;                         /* Format type value */
...
if (callback)
    callback (uptr, r ? SCPE_OK : SCPE_UNATT);
return r;
}

static bool sim_disk_no_autosize = false;

t_stat sim_disk_set_noautosize (int32_t flag, const char *cptr)
{
DEVICE *dptr;
uint32_t dev, unit, count = 0;
bool no_autosize = (flag != 0);
...
/* Enable asynchronous operation
 *
 * Now a no-op - disk I/O uses atomic pread/pwrite operations
 * which are inherently thread-safe without requiring async thread management.
 */
...
if (i < tbc)                 /* fill partial read with zeros */
    memset (&buf[i], 0, tbc-i);

if (i == 0 && tbc > 0)       /* Reading at or past EOF? */
    i = tbc;                /* return 0's which have already been filled in buffer */

if (sectsread) {
    size_t sectbytes = (i / ctx->sector_size) * ctx->sector_size;
    if (i > sectbytes)
...
if (sectswritten)
    *sectswritten = (t_seccnt)((i + ctx->sector_size - 1) / ctx->sector_size);
...
t_stat sim_disk_wrsect(UNIT *uptr, t_lba lba, uint8_t *buf, t_seccnt *sectswritten, t_seccnt sects)
{
    ...
    if (uptr->dynflags & UNIT_DISK_CHK) {
        DEVICE *dptr = find_dev_from_unit(uptr);
        uint32_t capac_factor = ((dptr->dwidth / dptr->aincr) >= 32) ? 8
                                : ((dptr->dwidth / dptr->aincr) == 16)
                                    ? 2
                                    : 1; /* capacity units (quadword: 8, word: 2, byte: 1) */
        t_lba total_sectors =
            (t_lba)((uptr->capac * capac_factor) / (ctx->sector_size / ((dptr->flags & DEV_SECTORS) ? 512 : 1)));
        t_lba sect;
        ...
        for (sect = 0; sect < sects; sect++) {
            ...
            if (sect_error) {
                ...
                sim_printf("\n%s: Write Address Verification Error on lbn %d(0x%X) of %d(0x%X).\n", sim_uname(uptr),
                           (int)(lba + sect), (int)(lba + sect), (int)total_sectors, (int)total_sectors);
                dptr->dctrl = 0xFFFFFFFF;
                sim_deb = save_sim_deb ? save_sim_deb : stdout;
                sim_disk_data_trace(uptr, buf + sect * ctx->sector_size, lba + sect, ctx->sector_size, "Found", true,
                                    1);
    ...
    case DKUF_F_VHD: /* VHD format */
        if (!sim_end && (ctx->xfer_element_size != sizeof(char))) {
            ...
            if (NULL == tbuf)
                return SCPE_MEM;
            sim_buf_copy_swapped(tbuf, buf, ctx->xfer_element_size,
                                 (sects * ctx->sector_size) / ctx->xfer_element_size);
            buf = tbuf;
        }
    ...
    if ((f == DKUF_F_RAW) && ((ctx->sector_size & (ctx->storage_sector_size - 1)) != 0) && /* NOT Sector Aligned */
        (((lba * ctx->sector_size) & (ctx->storage_sector_size - 1)) != 0 ||
         ((sects * ctx->sector_size) & (ctx->storage_sector_size - 1)) != 0)) {
        /* Unaligned and/or partial sector transfers in RAW mode require special handling */
        size_t tbufsize = sects * ctx->sector_size + 2 * ctx->storage_sector_size;
        t_offset ssaddr = (lba * (t_offset)ctx->sector_size) & ~(t_offset)(ctx->storage_sector_size - 1);
        ...
        sim_buf_copy_swapped(tbuf + soffset, buf, ctx->xfer_element_size,
                             (sects * ctx->sector_size) / ctx->xfer_element_size);
        r = sim_os_disk_write(uptr, ssaddr, tbuf, &byteswritten,
                              (soffset + (sects * ctx->sector_size) + ctx->storage_sector_size - 1) &
                                  ~(ctx->storage_sector_size - 1));
...
switch (f) {                                            /* case on format */
    ...
    case DKUF_F_VHD:                                    /* Virtual Disk */
        sim_vhd_disk_flush (sim_disk_get_vhd_handle(uptr->disk_image_file));
        break;
    ...
    case DKUF_F_VHD:                                    /* VHD format */
        {
            ...
            strlcpy ((char *)f->DriveType, sim_vhd_disk_get_dtype (sim_disk_get_vhd_handle(uptr->disk_image_file), &f->SectorSize, &f->TransferElementSize, (char *)f->CreatingSimulator, &creation_time), sizeof (f->DriveType));
            ...
            if ((f->SectorSize == 0) || (NtoHl (f->SectorSize) == 0x00020000)) {  /* Old or mangled format VHD footer */
                sim_vhd_disk_set_dtype (sim_disk_get_vhd_handle(uptr->disk_image_file), (char *)f->DriveType, ctx->sector_size, ctx->xfer_element_size);
                sim_vhd_disk_get_dtype (sim_disk_get_vhd_handle(uptr->disk_image_file), &f->SectorSize, &f->TransferElementSize, (char *)f->CreatingSimulator, NULL);
                f->SectorSize = NtoHl (f->SectorSize);
                f->TransferElementSize = NtoHl (f->TransferElementSize);
                }
            memset (f->CreationTime, 0, sizeof (f->CreationTime));
            strlcpy ((char*)f->CreationTime, ctime (&creation_time), sizeof (f->CreationTime));
            container_size = sim_vhd_disk_size (sim_disk_get_vhd_handle(uptr->disk_image_file));
...
#endif
SIM_FILE_HANDLE (*open_function)(const char *filename, const char *mode) = sim_fopen_handle;
SIM_FILE_HANDLE (*create_function)(const char *filename, t_offset desiredsize) = NULL;
t_stat (*storage_function)(SIM_FILE_HANDLE file, uint32_t *sector_size, uint32_t *removable, uint32_t *is_cdrom) = NULL;
...
if (sim_switches & SWMASK ('F')) {                      /* format spec? */
    ...
    auto_format = true;
    GET_SWITCHES (cptr);                                /* get any switches that follow the format spec */
    }
uptr->dynflags &= ~UNIT_VOLATILE;
...
if (sim_switches & SWMASK ('D')) {                      /* create difference disk? */
    ...
    if (*cptr == 0)                                     /* must be more */
        return SCPE_2FARG;
    vhd = sim_disk_make_vhd_handle(sim_vhd_disk_create_diff (gbuf, cptr));
    if (SIM_HANDLE_IS_VALID(vhd)) {
        sim_vhd_disk_close (sim_disk_get_vhd_handle(vhd));
...
if (sim_switches & SWMASK ('C')) {                      /* create new disk container & copy contents? */
    ...
    SIM_FILE_HANDLE dest;
    int saved_sim_switches = sim_switches;
    int32_t saved_sim_quiet = sim_quiet;
    uint32_t saved_fmt = DK_GET_FMT (uptr);                 /* save destination format */
    ...
    sim_quiet = true;
    /* First open the source of the copy operation - use AUTO format to detect source type */
    ...
    if (r != SCPE_OK) {
        ...
        /* Restore destination format by clearing and setting format bits */
        uptr->flags = (uptr->flags & ~DKUF_FMT) | (saved_fmt << DKUF_V_FMT);
    ...
    if (strcmp ("VHD", dest_fmt) == 0)
        dest = sim_disk_make_vhd_handle(sim_vhd_disk_create (gbuf, ((t_offset)uptr->capac)*capac_factor*((dptr->flags & DEV_SECTORS) ? 512 : 1)));
    else
        dest = sim_fopen_handle (gbuf, "wb+");
    if (SIM_HANDLE_IS_INVALID(dest)) {
        ...
        if (!copy_buf) {
            if (strcmp ("VHD", dest_fmt) == 0)
                sim_vhd_disk_close (sim_disk_get_vhd_handle(dest));
            else
                sim_fclose_handle (dest);
            (void)remove (gbuf);
            sim_disk_detach (uptr);
            return SCPE_MEM;
            }
    ...
    if (sim_switches & SWMASK ('M')) {                 /* merge difference disk? */
        ...
        vhd = sim_disk_make_vhd_handle(sim_vhd_disk_merge (gbuf, &Parent));
        if (SIM_HANDLE_IS_VALID(vhd)) {
            t_stat r;
...
if (ramdisk_attach) {
    ...
    case DKUF_F_AUTO:                                   /* SIMH format */
        auto_format = true;
        uptr->disk_image_file = sim_disk_make_vhd_handle(sim_vhd_disk_open (cptr, "rb"));
        if (SIM_HANDLE_IS_VALID(uptr->disk_image_file)) { /* Try VHD */
            sim_disk_set_fmt (uptr, 0, "VHD", NULL);    /* set file format to VHD */
            sim_vhd_disk_close (sim_disk_get_vhd_handle(uptr->disk_image_file));         /* close vhd file*/
            sim_vhd_opaque_init(&uptr->disk_image_file);
            open_function = (SIM_FILE_HANDLE (*)(const char *, const char *))sim_vhd_disk_open;
            break;
            }
        while (tmp_size < sector_size)
            tmp_size <<= 1;
        if (tmp_size ==  sector_size) {                     /* Power of 2 sector size can do RAW */
            uptr->disk_image_file = sim_os_disk_open_raw (cptr, "rb");
            if (SIM_HANDLE_IS_VALID(uptr->disk_image_file)) {
                ...
                sim_file_handle_init(&uptr->disk_image_file);
                break;
                }
            }
    ...
    case DKUF_F_STD:                                    /* SIMH format */
        uptr->disk_image_file = sim_disk_make_vhd_handle(sim_vhd_disk_open (cptr, "rb"));
        if (SIM_HANDLE_IS_VALID(uptr->disk_image_file)) { /* Try VHD first */
            sim_disk_set_fmt (uptr, 0, "VHD", NULL);    /* set file format to VHD */
            sim_vhd_disk_close (sim_disk_get_vhd_handle(uptr->disk_image_file));         /* close vhd file*/
            sim_file_handle_init(&uptr->disk_image_file);
            open_function = (SIM_FILE_HANDLE (*)(const char *, const char *)) sim_vhd_disk_open;
    ...
    case DKUF_F_VHD:                                    /* VHD format */
        open_function = (SIM_FILE_HANDLE (*)(const char *, const char *))sim_vhd_disk_open;
        create_function = (SIM_FILE_HANDLE (*)(const char *, t_offset))sim_vhd_disk_create;
        storage_function = sim_os_disk_info_raw;
        break;
    case DKUF_F_RAW:                                    /* Raw Physical Disk Access */
        uptr->disk_image_file = sim_disk_make_vhd_handle(sim_vhd_disk_open (cptr, "rb"));
...
if (ramdisk_attach) {
    ...
    if (SIM_HANDLE_IS_INVALID(uptr->disk_image_file))                          /* open fail? */
...
if ((DK_GET_FMT (uptr) == DKUF_F_VHD) || (ctx->footer)) {
    ...
    const char *container_dtype = ctx->footer ? (char *)ctx->footer->DriveType : sim_vhd_disk_get_dtype (sim_disk_get_vhd_handle(uptr->disk_image_file), &container_sector_size, &container_xfer_element_size, created_name, NULL);
    ...
    if ((DK_GET_FMT (uptr) == DKUF_F_VHD) && created && dtype) {
        sim_vhd_disk_set_dtype (sim_disk_get_vhd_handle(uptr->disk_image_file), dtype, ctx->sector_size, ctx->xfer_element_size);
        (void)get_disk_footer (uptr);
        ...
        if ((strcmp (container_dtype, dtype) == 0) ||
            ...
            if (strcmp (container_dtype, dtype) != 0) {
                if ((drivetypes != NULL) || (!dontchangecapac)) { /* Autosize allowed */
                    if (container_dtype[0] != '\0') {  /* Only set if container has a drive type */
...
{
struct disk_context *ctx;
int (*close_function)(SIM_FILE_HANDLE f);
SIM_FILE_HANDLE fileref;
bool auto_format;
...
switch (DK_GET_FMT (uptr)) {                            /* case on format */
    case DKUF_F_STD:                                    /* Simh */
        close_function = sim_fclose_handle;
        break;
    case DKUF_F_VHD:                                    /* Virtual Disk */
        close_function = sim_vhd_disk_close_wrapper;
        break;
    case DKUF_F_RAW:                                    /* Physical */
...
static SIM_FILE_HANDLE sim_os_disk_open_raw (const char *rawdevicename, const char *openmode)
{
HANDLE Handle;
...
if (Handle != INVALID_HANDLE_VALUE) {
    SIM_FILE_HANDLE temp_handle = sim_disk_make_native_handle(Handle);
    if ((sim_os_disk_info_raw (temp_handle, NULL, NULL, &is_cdrom)) ||
...
{
if (!CloseHandle (sim_disk_get_native_handle(f))) {
    _set_errno_from_status (GetLastError ());
    return EOF;
    }
return 0;
...
{
FlushFileBuffers (sim_disk_get_native_handle(f));
}

static t_offset sim_os_disk_size_raw (SIM_FILE_HANDLE Disk)
{
DWORD IoctlReturnSize;
LARGE_INTEGER Size;

if (GetFileSizeEx(sim_disk_get_native_handle(Disk), &Size))
...
#ifdef IOCTL_STORAGE_READ_CAPACITY
{
    ...
    if (DeviceIoControl(sim_disk_get_native_handle(Disk),     /* handle to volume */
...
static t_stat sim_os_disk_unload_raw (SIM_FILE_HANDLE Disk)
{
...
if (Removable) {
    if (!DeviceIoControl(sim_disk_get_native_handle(Disk),    /* handle to disk */
...
static bool sim_os_disk_isavailable_raw (SIM_FILE_HANDLE Disk)
{
#ifdef IOCTL_STORAGE_EJECT_MEDIA
DWORD BytesReturned;
uint32_t Removable = 0;
...
static t_stat sim_os_disk_info_raw (SIM_FILE_HANDLE Disk, uint32_t *sector_size, uint32_t *removable, uint32_t *is_cdrom)
{
DWORD IoctlReturnSize;
STORAGE_DEVICE_NUMBER Device;

ZeroMemory (&Device, sizeof (Device));
DeviceIoControl(sim_disk_get_native_handle(Disk),        /* handle to volume */
...
if (ReadFile (sim_disk_get_native_handle(uptr->disk_image_file), buf, (DWORD)bytes, (LPDWORD)bytesread, &pos))
    return SCPE_OK;
...
pos.Offset = (DWORD)addr;
pos.OffsetHigh = (DWORD)(addr >> 32);
if (WriteFile (sim_disk_get_native_handle(uptr->disk_image_file), buf, bytes, (LPDWORD)byteswritten, &pos))
    return SCPE_OK;
_set_errno_from_status (GetLastError ());
return SCPE_IOERR;
}
...
static t_stat sim_os_disk_unload_raw (SIM_FILE_HANDLE f)
{
...
static t_stat sim_os_disk_info_raw (SIM_FILE_HANDLE f, uint32_t *sector_size, uint32_t *removable, uint32_t *is_cdrom)
{
...
static t_stat ReadFilePosition(SIM_FILE_HANDLE File, void *buf, size_t bufsize, uint32_t *bytesread, uint64 position)
{
size_t bytes_read;
...
bytes_read = sim_disk_pread (File, buf, bufsize, (t_offset)position);
if (bytes_read == (size_t)-1)
    return SCPE_IOERR;

if (bytesread)
    *bytesread = (uint32_t)bytes_read;
return SCPE_OK;
}

static t_stat WriteFilePosition(SIM_FILE_HANDLE File, void *buf, size_t bufsize, uint32_t *byteswritten, uint64 position)
{
size_t bytes_written;
...
bytes_written = sim_disk_pwrite (File, buf, bufsize, (t_offset)position);
if (bytes_written == (size_t)-1)
    return SCPE_IOERR;

if (byteswritten)
    *byteswritten = (uint32_t)bytes_written;
return SCPE_OK;
}
...
{
SIM_FILE_HANDLE File = SIM_FILE_HANDLE_INIT;
uint64 position;
...
if (aBAT)
    *aBAT = NULL;
File = sim_fopen_handle (szVHDPath, "rb");
if (SIM_HANDLE_IS_INVALID(File)) {
    Return = errno;
    goto Return_Cleanup;
    }
if (ModifiedTimeStamp) {
    if (sim_stat (szVHDPath, &statb)) {
        Return = errno;
        goto Return_Cleanup;
        }
    else
...
position = sim_fsize_handle (File);
if (((int64)position) == -1) {
    Return = errno;
    goto Return_Cleanup;
    }
...
Return_Cleanup:
if (SIM_HANDLE_IS_VALID(File))
    sim_fclose_handle(File);
if (aBAT && (0 != Return)) {
    free (*aBAT);
    *aBAT = NULL;
    }
...
if (NtoHl (hVHD->Footer.DiskType) == VHD_DT_Fixed) {
    ...
    position = sim_fsize_handle (hVHD->File);
    if (((int64)position) == -1) {
        Status = errno;
        goto Cleanup_Return;
        }
...
static VHDHANDLE sim_vhd_disk_open (const char *szVHDPath, const char *DesiredAccess)
    {
    ...
    if (NtoHl (hVHD->Footer.DiskType) == VHD_DT_Differencing) {
        ...
        hVHD->Parent = sim_vhd_disk_open (hVHD->ParentVHDPath, "rb");
        if (!hVHD->Parent) {
            Status = errno;
            goto Cleanup_Return;
            }
    ...
    if (SIM_HANDLE_IS_INVALID(hVHD->File)) {
        Status = errno;
        goto Cleanup_Return;
        }
Cleanup_Return:
...
static VHDHANDLE sim_vhd_disk_merge (const char *szVHDPath, char **ParentVHD)
    ...
    Parent = sim_vhd_disk_open (hVHD->ParentVHDPath, "rb+");
    if (!Parent) {
        Status = errno;
        goto Cleanup_Return;
        }
    ...
    if (BlockNumber < NtoHl (hVHD->Dynamic.MaxTableEntries)) {
        ...
        sim_fclose_handle (hVHD->File);
        hVHD->File = SIM_FILE_HANDLE_INIT;
        (void)remove (szVHDPath);
        *ParentVHD = strdup (hVHD->ParentVHDPath);
        }
Cleanup_Return:
    free (BlockData);
    if (SIM_HANDLE_IS_VALID(hVHD->File))
        sim_fclose_handle (hVHD->File);
    if (Status) {
        free (hVHD->BAT);
        free (hVHD);
        hVHD = NULL;
        sim_vhd_disk_close (Parent);
        }
    else {
        free (hVHD->BAT);
        free (hVHD);
...
if (hVHD != NULL) {
    if (hVHD->Parent)
        sim_vhd_disk_close (hVHD->Parent);
    free (hVHD->BAT);
    if (SIM_HANDLE_IS_VALID(hVHD->File)) {
        sim_fflush_handle (hVHD->File);
        sim_fclose_handle (hVHD->File);
        }
    free (hVHD);
    return 0;
...
if ((hVHD != NULL) && SIM_HANDLE_IS_VALID(hVHD->File))
    sim_fflush_handle (hVHD->File);
}
...
if (SIM_HANDLE_IS_VALID(File)) {
    sim_fclose_handle (File);
    File = SIM_FILE_HANDLE_INIT;
    Status = EEXIST;
    goto Cleanup_Return;
    }
File = sim_fopen_handle (szVHDPath, "wb+");
if (SIM_HANDLE_IS_INVALID(File)) {
    Status = errno;
    goto Cleanup_Return;
    }
...
if (Status) {
    ...
    hVHD = sim_vhd_disk_open (szVHDPath, "rb+");
    if (!hVHD)
        Status = errno;
    }
errno = Status;
...
static VHDHANDLE sim_vhd_disk_create (const char *szVHDPath, t_offset desiredsize)
{
return sim_CreateVirtualDisk (szVHDPath, (uint32_t)(desiredsize/512), 0,
                                      (sim_switches & SWMASK ('X')) != 0);
}

static VHDHANDLE sim_vhd_disk_create_diff (const char *szVHDPath, const char *szParentVHDPath)
{
return CreateDifferencingVirtualDisk (szVHDPath, szParentVHDPath);
}
...
if (BytesRead)
    *BytesRead = 0;
if (!hVHD || SIM_HANDLE_IS_INVALID(hVHD->File)) {
    errno = EBADF;
    return SCPE_IOERR;
    }
if (BytesToRead == 0)
...
static t_stat sim_vhd_disk_rdsect (UNIT *uptr, t_lba lba, uint8_t *buf, t_seccnt *sectsread, t_seccnt sects)
{
VHDHANDLE hVHD = sim_disk_get_vhd_handle(uptr->disk_image_file);
struct disk_context *ctx = (struct disk_context *)uptr->disk_ctx;
...
static t_stat sim_vhd_disk_clearerr (UNIT *uptr)
{
/* With handle-based I/O, errors are per-operation, not persistent */
(void)uptr;
return SCPE_OK;
}
...
while (BytesToWrite && (r == SCPE_OK)) {
    ...
    if (hVHD->BAT[BlockNumber] == VHD_BAT_FREE_ENTRY) {
        ...
        BlockOffset = sim_fsize_handle (hVHD->File);
...
static bool sim_disk_check_attached_container (const char *filename)
{
DEVICE *dptr;
UNIT *uptr;
uint32_t i, j;
...
if (info->flag) {        /* zap type */
    ...
    vhd_container = sim_disk_make_vhd_handle(sim_vhd_disk_open (FullPath, "r"));
    if (SIM_HANDLE_IS_VALID(vhd_container)) {
        sim_vhd_disk_close (sim_disk_get_vhd_handle(vhd_container));
...
if (info->flag == 0) {
    ...
    t_offset (*size_function)(SIM_FILE_HANDLE file);
    int (*close_function)(SIM_FILE_HANDLE f);
    SIM_FILE_HANDLE container;
    t_offset container_size;
    ...
    container = sim_disk_make_vhd_handle(sim_vhd_disk_open (FullPath, "r"));
    if (SIM_HANDLE_IS_INVALID(container)) {
        sim_disk_set_fmt (uptr, 0, "SIMH", NULL);
        container = sim_fopen_handle (FullPath, "rb+");
        close_function = sim_fclose_handle;
        size_function = sim_fsize_handle;
        }
    else {
        close_function = sim_vhd_disk_close_wrapper;
        size_function = sim_vhd_disk_size_wrapper;
        }
    if (SIM_HANDLE_IS_VALID(container)) {
...
t_stat sim_disk_info_cmd (int32_t flag, const char *cptr)
{
