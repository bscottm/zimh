...
static int do_poll(sim_slirp_network *slirp, int ms_timeout, struct timeval *timeout);
static void report_error(sim_slirp_network *slirp);

/* Retrieve the debug mask for an element in the augmented debug table: */
static inline uint32_t slirp_dbg_mask(const sim_slirp_network *slirp, size_t flag)
{
...
#else
    slirp_pollfds_fill(slirp->slirp_cxn, &slirp_timeout, add_poll_callback, slirp);
#endif
}
...
#  else
    if (slirp->max_fd != SIM_INVALID_MAX_FD) {
        retval = select(slirp->max_fd + 1, &slirp->readfds, &slirp->writefds, &slirp->exceptfds, &timeout);
        if (retval > 0)
        ...
        if (retval > 0) {
            ...
            if (sim_deb != NULL && slirp->dptr != NULL && (slirp->dptr->dctrl & dbg_mask)) {
                ...
                for (i = 0; i < (size_t) slirp->fd_idx && sim_deb != NULL; ++i) {
                    fprintf(sim_deb, "[%3zu] fd: %04" PRIsocket ", events: %04x\n",
...
static void report_error(sim_slirp_network *slirp)
{
    ...
    sim_messagef(SCPE_IERR, "sim_slirp_select(): do_poll() -- WSAGetLastError() %d (%s)", wsaError, wsaMsgBuffer);
...
void register_poll_socket(slirp_os_socket fd, void *opaque)
{
    ...
    if (aio_enabled_and_active() && sim_atomic_inc(&slirp->n_sockets) == 1) {
...
static int add_poll_callback(slirp_os_socket fd, int events, void *opaque)
{
    ...
    char prefix[128];
    size_t i;
    const uint32_t dbg_mask = slirp_dbg_mask(slirp, DBG_POLL);
...
#  endif
    ...
    if (sim_deb != NULL && slirp->dptr != NULL && (slirp->dptr->dctrl & dbg_mask)) {
        sprintf(prefix, "add_poll_callback(%" PRIsocket ")/poll (0x%04x)", fd, events);
...
{
    const sim_slirp_network *slirp = (sim_slirp_network *) opaque;
    int event = 0;
    char prefix[128];
    const uint32_t dbg_mask = slirp_dbg_mask(slirp, DBG_POLL);
    ...
    if (idx >= 0 && (size_t) idx < slirp->lut_copy_in_use) {
        ...
        if (sim_deb != NULL && slirp->dptr != NULL && (slirp->dptr->dctrl & dbg_mask)) {
            sprintf(prefix, "get_events_callback(%" PRIsocket ")/select (0x%04x)", fd, event & event_mask);
    ...
    if (idx >= 0 && (size_t) idx < slirp->fd_idx) {
        ...
        if (sim_deb != NULL && slirp->dptr != NULL && (slirp->dptr->dctrl & dbg_mask)) {
            sprintf(prefix, "get_events_callback(%" PRIsocket ")/poll (0x%04x)", slirp->fds[idx].fd, event);
