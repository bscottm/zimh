...
#    define SZ_LOAD(sz, v, mb, j)                                                                                      \
        if (sz == sizeof(uint8_t))                                                                                     \
            v = *(((uint8_t *)mb) + ((uint32_t)j));                                                                    \
        else if (sz == sizeof(uint16_t))                                                                               \
            v = *(((uint16_t *)mb) + ((uint32_t)j));                                                                   \
        else if (sz == sizeof(uint32_t))                                                                               \
            v = *(((uint32_t *)mb) + ((uint32_t)j));                                                                   \
        else                                                                                                           \
            v = *(((uint64_t *)mb) + ((uint32_t)j));
#    define SZ_STORE(sz, v, mb, j)                                                                                     \
        if (sz == sizeof(uint8_t))                                                                                     \
            *(((uint8_t *)mb) + j) = (uint8_t)v;                                                                       \
        else if (sz == sizeof(uint16_t))                                                                               \
            *(((uint16_t *)mb) + ((uint32_t)j)) = (uint16_t)v;                                                         \
        else if (sz == sizeof(uint32_t))                                                                               \
            *(((uint32_t *)mb) + ((uint32_t)j)) = (uint32_t)v;                                                         \
        else                                                                                                           \
            *(((uint64_t *)mb) + ((uint32_t)j)) = v;
#else
#    define SZ_LOAD(sz, v, mb, j)                                                                                      \
        if (sz == sizeof(uint8_t))                                                                                     \
            v = *(((uint8_t *)mb) + ((uint32_t)j));                                                                    \
...
#    define SZ_STORE(sz, v, mb, j)                                                                                     \
        if (sz == sizeof(uint8_t))                                                                                     \
            *(((uint8_t *)mb) + ((uint32_t)j)) = (uint8_t)v;                                                           \
        else if (sz == sizeof(uint16_t))                                                                               \
            *(((uint16_t *)mb) + ((uint32_t)j)) = (uint16_t)v;                                                         \
        else                                                                                                           \
            *(((uint32_t *)mb) + ((uint32_t)j)) = v;
#endif

static DEBTAB scp_debug[] = {{"EVENT", SIM_DBG_EVENT, "Event Dispatch Activities"},
...
DEVICE sim_scp_dev = {"SCP-PROCESS",
                      scp_test_units,
                      NULL,
                      NULL,
                      4,
...
t_stat exdep_reg_loop(FILE *ofile, SCHTAB *schptr, int32_t flag, const char *cptr, REG *lowr, REG *highr, uint32_t lows,
                      uint32_t highs);
...
t_stat exdep_addr_loop(FILE *ofile, SCHTAB *schptr, int32_t flag, const char *cptr, t_addr low, t_addr high,
                       DEVICE *dptr, UNIT *uptr);
t_stat ex_addr(FILE *ofile, int32_t flag, t_addr addr, DEVICE *dptr, UNIT *uptr, int32_t dfltinc);
t_stat dep_addr(int32_t flag, const char *cptr, t_addr addr, DEVICE *dptr, UNIT *uptr, int32_t dfltinc);
...
static bool sim_if_cmd[MAX_DO_NEST_LVL + 1];
static bool sim_if_cmd_last[MAX_DO_NEST_LVL + 1];
static bool sim_if_result[MAX_DO_NEST_LVL + 1];
static bool sim_if_result_last[MAX_DO_NEST_LVL + 1];
static bool sim_cptr_is_action[MAX_DO_NEST_LVL + 1];
...
DEVICE sim_step_dev = {"INT-STEP", &sim_step_unit,
                       NULL,       NULL,
                       1,          0,
                       0,          0,
                       0,          0,
...
DEVICE sim_runlimit_dev = {"INT-RUNLIMIT",
                           &sim_runlimit_unit,
                           NULL,
                           NULL,
                           1,
...
static REG sim_flush_reg[] = {
    {DRDATAD(FLUSH_INTERVAL, sim_flush_interval, 32, "Periodic Buffer Flush Interval (seconds)")}, {NULL}};

static UNIT sim_flush_unit = {UDATA(&flush_svc, UNIT_IDLE, 0)};
DEVICE sim_flush_dev = {"INT-FLUSH",
                        &sim_flush_unit,
                        sim_flush_reg,
                        NULL,
                        1,
...
static CTAB cmd_table[] = {{"RESET", &reset_cmd, 0, HLP_RESET, NULL, NULL},
...
static CTAB set_glob_tab[] = {{"CONSOLE", &sim_set_console, 0, HLP_SET_CONSOLE},
...
static C1TAB set_dev_tab[] = {{"OCTAL", &set_dev_radix, 8},
...
static C1TAB set_unit_tab[] = {{"ENABLED", &set_unit_enbdis, 1},
...
static SHTAB show_glob_tab[] = {{"CONFIGURATION", &show_config, 0, HLP_SHOW_CONFIG},
...
static SHTAB show_dev_tab[] = {{"RADIX", &show_dev_radix, 0},         {"DEBUG", &show_dev_debug, 0},
                               {"MODIFIERS", &show_dev_modifiers, 0}, {"NAMES", &show_dev_logicals, 0},
                               {"SHOW", &show_dev_show_commands, 0},  {NULL, NULL, 0}};

static SHTAB show_unit_tab[] = {{"DEBUG", &show_dev_debug, 1}, {NULL, NULL, 0}};
    ...
    *cbuf = 0;               /* init arg buffer */
    ...
    if (docmdp) {
        if (cptr && (sizeof(nbuf) > strlen(cptr) + strlen("/simh.ini") + 3)) {
            snprintf(nbuf, sizeof(nbuf), "\"%s%s%ssimh.ini\"", cptr2 ? cptr2 : "", cptr,
                     strchr(cptr, '/') ? "/" : "\\");
...
t_stat process_stdin_commands(t_stat stat, char *argv[], bool do_called)
{
    ...
    while (stat != SCPE_EXIT) {    /* in case exit */
        ...
        if (!stat_nomessage) {                                  /* displaying message status? */
            ...
            else if (stat >= SCPE_BASE)                         /* error? */
                sim_printf("%s\n", sim_error_text(stat));
        }
        if ((sim_on_check[sim_do_depth]) && (stat != SCPE_OK)) {
    ...
    if (do_called && cmdp && (cmdp->action == &return_cmd) && (0 != *cptr)) { /* return command with argument? */
...
{
    return (sim_argv && sim_argv[0]) ? sim_argv[0] : NULL;
}

bool scp_has_oline(void)
{
    return sim_oline != NULL;
}
...
t_stat help_cmd(int32_t flag, const char *cptr)
{
    ...
    if (*cptr) {
        ...
        if ((0 == strncmp(gbuf, "DEV", 3)) && (0 == MATCH_CMD(gbuf, "DEVICE"))) {
            explicit_device = true;
            cptr = get_glyph(cptr, gbuf, 0);
        }
        dptr = find_unit(gbuf, &uptr);
        if ((dptr == NULL) && ((dptr = find_dev(gbuf)) == NULL)) {
            if (explicit_device) {
                int i;
                ...
                if (*cptr) {
                    if ((cmdp->action == &set_cmd) || (cmdp->action == &show_cmd)) {
                        ...
                        if (dptr != NULL) {
                            ...
                            if (sim_log && (!sim_oline))
                                help_dev_help(sim_log, dptr, uptr, flag | SCP_HELP_FLAT,
                                              (cmdp->action == &set_cmd) ? "SET" : "SHOW");
                            return r;
                        }
                        if (cmdp->action == &set_cmd) { /* HELP SET xxx (not device or unit) */
                            if ((cmdp = find_ctab(set_glob_tab, gbuf)) && (cmdp->help))
                ...
                if (cmdp->help) {
                    if (strcmp(cmdp->name, "HELP") == 0) {
                        ...
                        for (i = 0; (dptr = sim_devices[i]) != NULL; i++) {
                            ...
                            if (dptr->attach_help || (DEV_TYPE(dptr) == DEV_MUX) || (DEV_TYPE(dptr) == DEV_DISK) ||
                            ...
                            if (dptr->modifiers) {
                                ...
                                for (mptr = dptr->modifiers; mptr->pstring != NULL; mptr++) {
                                    if (mptr->help) {
                                        sim_printf(
                                            "h{elp} %s SET\t\t display help for device %s SET commands (modifiers)\n",
                                            dptr->name, dptr->name);
                                        break;
                                    }
                                }
                        ...
                        if (((cmdp->action == &exdep_cmd) || (0 == strcmp(cmdp->name, "BOOT"))) && sim_dflt_dev->help) {
...
static char *do_position(void)
{
    ...
    snprintf(cbuf, sizeof(cbuf), "%s%s%s-%d", sim_do_filename[sim_do_depth], sim_do_label[sim_do_depth] ? "::" : "",
             sim_do_label[sim_do_depth] ? sim_do_label[sim_do_depth] : "", sim_goto_line[sim_do_depth]);
    return cbuf;
}
...
t_stat do_cmd_label(int32_t flag, const char *fcptr, const char *label)
{
    ...
    int32_t saved_sim_do_echo = sim_do_echo, saved_sim_show_message = sim_show_message,
            saved_sim_on_inherit = sim_on_inherit, saved_sim_quiet = sim_quiet;
    bool staying;
    ...
    strlcpy(sim_do_filename[sim_do_depth], strcasecmp(cbuf, "<stdin>") ? c : cbuf,
            sizeof(sim_do_filename[sim_do_depth])); /* stash away full path of do file name for possible use by 'call'
                                                       command */
    free(c);
    ...
    do {
        ...
        if ((cmdp = find_cmd(gbuf))) {                           /* lookup command */
            ...
            } else if (cmdp->action == &shift_cmd)               /* SHIFT command */
        ...
        if (((stat != SCPE_OK) && (stat != SCPE_EXPECT)) ||
            ((cmdp->action != &return_cmd) && (cmdp->action != &goto_cmd) && (cmdp->action != &on_cmd) &&
             (cmdp->action != &echo_cmd) && (cmdp->action != &echof_cmd) && (cmdp->action != &sleep_cmd)))
            sim_last_cmd_stat = stat;                       /* save command error status */
        switch (stat) {
        case SCPE_AFAIL:
        ...
        if (!stat_nomessage) {          /* report error if not suppressed */
            ...
            else if (stat >= SCPE_BASE) /* report error if not suppressed */
                sim_printf("%s\n", sim_error_text(stat));
        }
        if (stat == SCPE_EXPECT)        /* EXPECT status is non actionable */
            stat = SCPE_OK;             /* so adjust it to SCPE_OK */
        if (staying && (sim_on_check[sim_do_depth]) && (stat != SCPE_OK)) {
            if ((stat <= SCPE_MAX_ERR) && sim_on_actions[sim_do_depth][stat])
                sim_brk_setact(sim_on_actions[sim_do_depth][stat]);
            else
                sim_brk_setact(sim_on_actions[sim_do_depth][0]);
...
int sim_cmp_string(const char *s1, const char *s2)
{
    ...
    if ((ep1 != s1 + strlen(s1) - 1) || (ep2 != s2 + strlen(s2) - 1))
        return (strlen(s1) == strlen(s2)) ? strncmp(s1 + 1, s2 + 1, strlen(s1) - 2) : strcmp(s1, s2);
    if (v1 == v2)
        return 0;
    if (v1 < v2)
        return -1;
...
/* Assert command
   ...
   Syntax: IF {NOT} {<dev>} <reg>{<logical-op><value>}<conditional-op><value> commandtoprocess{;
   additionalcommandtoprocess}...
   ...
   Syntax: IF {-i} {NOT} "<string1>"|EnvVarName1 <compare-op> "<string2>"|EnvVarName2 commandtoprocess{;
   additionalcommandtoprocess}...
...
static const char *_get_string(const char *iptr, char *optr, char mchar)
{
    ...
    tptr = (const char *)get_glyph_gen(iptr, optr, mchar, ((sim_switches & SWMASK('I')) != 0), true, '\\');
    if ((*optr != '"') && (*optr != '\'')) {
        ap = getenv(optr);
        if (!ap)
            return tptr;
...
t_stat assert_cmd(int32_t flag, const char *cptr)
{
    ...
    if (Exist || (*gbuf == '"') || (*gbuf == '\'')) { /* quoted string comparison? */
        ...
        if (!Exist) {
            ...
            result = ((compare_result == optr->aval) || (compare_result == optr->bval));
            if (optr->invert)
                result = !result;
        } else {
        ...
        if (*cptr == '(') {
            ...
            if (rptr) {                                            /* Handle register case */
                ...
                if (!get_asearch(gbuf, sim_dfdev ? sim_dfdev->dradix : sim_dflt_dev->dradix,
                                 &sim_staba) ||                    /* parse condition */
...
t_stat goto_cmd(int32_t flag, const char *fcptr)
{
    ...
    if ((NULL == sim_gotofile) || (0 == strcasecmp(sim_do_filename[sim_do_depth], "<stdin>")))
...
t_stat call_cmd(int32_t flag, const char *fcptr)
{
    ...
    if ('\0' == gbuf[0])
        return SCPE_ARG;         /* unspecified goto target */
    snprintf(cbuf, sizeof(cbuf), "%s%s%s %s", (NULL != strchr(sim_do_filename[sim_do_depth], ' ')) ? "\"" : "",
             sim_do_filename[sim_do_depth], (NULL != strchr(sim_do_filename[sim_do_depth], ' ')) ? "\"" : "", cptr);
...
t_stat on_cmd(int32_t flag, const char *cptr)
{
    ...
    cptr = get_glyph(cptr, gbuf, 0);
    if ('\0' == gbuf[0])
        return SCPE_ARG; /* unspecified condition */
    if (0 == strcmp("ERROR", gbuf))
        cond = 0;
    else {
        if (SCPE_OK != sim_string_to_stat(gbuf, &cond)) {
            if ((MATCH_CMD(gbuf, "CONTROL_C") == 0) || (MATCH_CMD(gbuf, "SIGINT") == 0))
...
t_stat set_on(int32_t flag, const char *cptr)
{
    if ((flag) && (cptr) && (*cptr)) {   /* Set ON with arg */
        ...
        if (((MATCH_CMD(gbuf, "INHERIT")) && (MATCH_CMD(gbuf, "NOINHERIT"))) || (*cptr))
            return SCPE_2MARG;
    ...
    if ((sim_do_depth != 0) && (NULL == sim_on_actions[sim_do_depth][0])) { /* default handler set? */
        sim_on_actions[sim_do_depth][0] = strdup("RETURN");                 /* No, so make "RETURN" */
        if (sim_on_actions[sim_do_depth][0] == NULL)
            return SCPE_MEM;
    }
    if ((sim_do_depth != 0) && (NULL == sim_on_actions[sim_do_depth][SCPE_AFAIL])) { /* handler set for AFAIL? */
            ...
            if ((mptr->mstring) &&                       /* match string */
                ...
                if (mptr->mask & MTAB_XTD) {             /* extended? */
                    ...
                    if ((mptr->valid) &&                                                      /* invalid? */
                        ...
                        return r;
                    uptr->flags = (uptr->flags & ~(mptr->mask)) | (mptr->match & mptr->mask); /* set new value */
                } /* end else xtd */
                break; /* terminate for */
            } /* end if match */
        } /* end for */
...
t_stat set_dev_debug(DEVICE *dptr, UNIT *uptr, int32_t flags, const char *cptr)
{
    ...
    if (cptr == NULL) { /* no arguments? */
        uint32_t debug_mask = enable ? (dptr->debflags ? enable_mask : 0xFFFFFFFFu) : 0;
...
t_stat show_cmd_fi(FILE *ofile, int32_t flag, const char *cptr)
{
    ...
    if ((*cptr == 0) || (*cptr == ';') || (*cptr == '#')) { /* now eol? */
        return (lvl == MTAB_VDV) ? show_device(ofile, dptr, 0) : show_unit(ofile, dptr, uptr, -1);
    }
    GET_SWITCHES(cptr);                                     /* get more switches */
...
static const char *_sim_name_prefix(const char *name, const char *prefix, size_t max_name_len)
{
    ...
    snprintf(nambuf, sizeof(nambuf), "%s%*s", prefix ? prefix : "",
             ((string_len <= 6) && (max_name_len <= 6)) ? -((int)(8 - prefix_len)) : -((int)(max_name_len + 2)),
             name ? name : "");
    return nambuf;
}
...
static void _set_dname_len(DEVICE *dptr)
{
    ...
    for (i = 0; i < dptr->numunits; i++) {
        ...
        if (((uptr->flags & UNIT_DIS) == 0) && (unit_name_len < strlen(sim_uname(uptr))))
...
t_stat show_device(FILE *st, DEVICE *dptr, int32_t flag)
{
    ...
    if (dptr->numunits == 0) {
        ...
        if (ucnt == 0) {
            ...
            fprint_sep(st, &toks);
            fprintf(st, "%d units\n", ucnt + udbl);
        } else if ((flag != 2) || !dptr->description || toks)
            fprintf(st, "\n");
        toks = 0;
    }
    if (flag)                              /* dev only? */
...
const char *sprint_capac(DEVICE *dptr, UNIT *uptr)
{
    ...
    if ((dptr->dwidth / dptr->aincr) > 8)
        ...
        width = "B";
    if ((psize < (kval * 10)) && (0 != (psize % kval))) {
        remfrac = 0.0;
        scale = "";
    } else if ((psize < (mval * 10)) && (0 != (psize % mval))) {
        scale = "K";
    ...
    if ((remfrac != 0.0) && (sim_switches & SWMASK('R'))) {
        ...
        snprintf(plast_char, sizeof(capac_buf) - (size_t)(plast_char - capac_buf), "%0.3f", remfrac);
...
void sim_fprint_regex_support(FILE *st)
{
    ...
    if (version && *version)
        fprintf(st, "\n        %s RegEx (Version %s) support for EXPECT commands", backend, version);
    else
        fprintf(st, "\n        %s RegEx support for EXPECT commands", backend);
}
...
#else
        fprintf(st, "\n        Simulator Build: %s%s %s", arch, build, S_xstr(SIM_BUILD_OS));
#endif
...
t_stat show_queue(FILE *st, DEVICE *dnotused, UNIT *unotused, int32_t flag, const char *cptr)
{
    ...
    if (cptr && (*cptr != 0))
        return SCPE_2MARG;
    if (sim_clock_queue == QUEUE_LIST_END)
        fprintf(st, "%s event queue empty, time = %.0f, executing %s %s/sec\n", sim_name, sim_vm_time,
                sim_fmt_numeric(sim_timer_inst_per_sec()), sim_vm_interval_units);
    else {
        const char *tim = "";
        ...
        for (uptr = sim_clock_queue; uptr != QUEUE_LIST_END; uptr = uptr->next) {
            if (uptr == &sim_step_unit)
            ...
            else if (sim_expect_is_unit(uptr))
                ...
                fprintf(st, "  Unknown");
            if (inst_per_sec != 0.0)
                tim = sim_fmt_secs(((_sim_activate_queue_time(uptr) - 1) / sim_timer_inst_per_sec()) +
                                   (uptr->usecs_remaining / 1000000.0));
            if (uptr->usecs_remaining)
                fprintf(st, " at %d plus %.0f usecs%s%s%s%s\n", _sim_activate_queue_time(uptr) - 1,
                        uptr->usecs_remaining, (*tim) ? " (" : "", tim, (*tim) ? " total)" : "",
                        (uptr->flags & UNIT_IDLE) ? " (Idle capable)" : "");
            else
                fprintf(st, " at %d%s%s%s%s\n", _sim_activate_queue_time(uptr) - 1, (*tim) ? " (" : "", tim,
                        (*tim) ? ")" : "", (uptr->flags & UNIT_IDLE) ? " (Idle capable)" : "");
        }
    }
...
t_stat show_on(FILE *st, DEVICE *dptr, UNIT *uptr, int32_t flag, const char *cptr)
{
    ...
    if (cptr && (*cptr != 0))
        return SCPE_2MARG; /* now eol? */
    if (flag < 0)
        lvl = sim_do_depth;
    else if (flag > sim_do_depth)
        return SCPE_ARG;
    else
        lvl = flag;
    if (flag == -1) {
...
t_stat show_all_mods(FILE *st, DEVICE *dptr, UNIT *uptr, int32_t flag, int32_t *toks)
{
    ...
    for (mptr = dptr->modifiers; mptr->mask != 0; mptr++) {
        if (mptr->pstring &&
            ((mptr->mask & MTAB_XTD) ? (MODMASK(mptr, flag) && !MODMASK(mptr, MTAB_NMO))
                          ...
                          void *context)
{
    ...
    if (local)
        sim_printf("%02d/%02d/%04d  %02d:%02d %s ", local->tm_mon + 1, local->tm_mday, 1900 + local->tm_year,
                   local->tm_hour % 12, local->tm_min, (local->tm_hour >= 12) ? "PM" : "AM");
...
t_stat dir_cmd(int32_t flg, const char *cptr)
{
    ...
    if (*cptr == '\0')
        ...
        if ((WildName[strlen(WildName) - 1] == '/') || (WildName[strlen(WildName) - 1] == '\\'))
                           ...
                           void *context)
{
    ...
    while ((NULL != fgets(lbuf, sizeof(lbuf) - 1, file)) && (lines++ < sim_type_line_count))
        sim_printf("%s", lbuf);
    fclose(file);
}
...
t_stat runlimit_cmd(int32_t flag, const char *cptr)
{
    ...
    if ((r != SCPE_OK) || (num == 0)) /* error? */
        return sim_messagef(SCPE_ARG, "Invalid argument: %s\n", gbuf);
    cptr = get_glyph(cptr, gbuf, 0);  /* get next glyph */
    if ((gbuf[0] == '\0') || (MATCH_CMD(gbuf, sim_vm_interval_units) == 0)) {
        ...
        struct {
            const char *name;
            double usec_factor;
        } time_units[] = {{"MICROSECONDS", 1.0},          {"USECONDS", 1.0},
                          {"SECONDS", 1000000.0},         {"MINUTES", 60 * 1000000.0},
                          {"HOURS", 60 * 60 * 1000000.0}, {NULL, 0.0}};
    ...
    if (sim_runlimit_switches & SWMASK('T')) {
        ...
        if (sim_host_speed_factor() > 1.0)
            sim_messagef(SCPE_OK, "Slow host - adjusting RUNLIMIT from %d %s to %.1f %s\n", num, units,
                         num * sim_host_speed_factor(), units);
...
#define WRITE_I(xx) sim_fwrite(&(xx), sizeof(xx), 1, sfile)
    ...
    for (device_count = 0; sim_devices[device_count]; device_count++)
        ;                                                              /* count devices */
        ...
        for (j = 0; j < dptr->numunits; j++) {
            ...
            if (uptr->flags & UNIT_ATT) {
                ...
                if ((uptr->flags & UNIT_BUF) &&       /* writable buffered */
                    ...
                    rewind(uptr->fileref);
                    sim_fwrite(uptr->filebuf, scp_device_data_size_bytes(dptr), cap, uptr->fileref);
                    fclose(uptr->fileref);                            /* flush data and state */
                    uptr->fileref = sim_fopen(uptr->filename, "rb+"); /* reopen r/w */
                }
            }
            fputc('\n', sfile);
            if (((uptr->flags & (UNIT_FIX + UNIT_ATTABLE)) == UNIT_FIX) && (dptr->examine != NULL) &&
                ...
                for (k = 0; k < high;) {                                                  /* loop thru mem */
                    zeroflg = true;
                    for (l = 0; (l < SRBSIZ) && (k < high); l++, k = k + (dptr->aincr)) { /* check for 0 block */
...
t_stat sim_rest(FILE *rfile)
{
    ...
    REG *rptr;
    struct stat rstat;
    bool force_restore = ((sim_switches & SWMASK('F')) != 0);
    bool dont_detach_attach = ((sim_switches & SWMASK('D')) != 0);
    bool suppress_warning = ((sim_switches & SWMASK('Q')) != 0);
    bool warned = false;

    sim_debug(SIM_DBG_RESTORE, &sim_scp_dev, "sim_rest (force=%d, dont_detach=%d, nowarnings=%d)\n", force_restore,
              dont_detach_attach, suppress_warning);
    sim_switches &= ~(SWMASK('F') | SWMASK('D') | SWMASK('Q')); /* remove digested switches */
#define READ_S(xx)                                                                                                     \
    if (read_line((xx), sizeof(xx), rfile) == NULL) {                                                                  \
        r = SCPE_IOERR;                                                                                                \
        goto Cleanup_Return;                                                                                           \
    }
#define READ_I(xx)                                                                                                     \
    if (sim_fread(&xx, sizeof(xx), 1, rfile) == 0) {                                                                   \
        r = SCPE_IOERR;                                                                                                \
        goto Cleanup_Return;                                                                                           \
    }
    ...
    for (;;) {                                /* device loop */
        ...
        if ((buf[0] != 0) && ((r = assign_device(dptr, buf)) != SCPE_OK)) {
            r = SCPE_INCOMP;
            goto Cleanup_Return;
        }
        ...
        if (!v32)
            flg =
                ((flg & DEV_UFMASK_31) << (DEV_V_UF - DEV_V_UF_31)) | (flg & ~DEV_UFMASK_31); /* [V3.2+] flags moved */
        ...
        for (;;) {                                                                            /* unit loop */
            ...
            uptr->flags = (uptr->flags & ~UNIT_RFLAGS) | (flg & UNIT_RFLAGS); /* restore */
            ...
            if (high > 0) { /* [V2.5+] any memory? */
                if (((uptr->flags & (UNIT_FIX + UNIT_ATTABLE)) != UNIT_FIX) || (dptr->deposit == NULL)) {
                ...
                if (high != old_capac) {     /* size change? */
                    ...
                    if ((dptr->flags & DEV_DYNM) &&
                        ((dptr->msize == NULL) || (dptr->msize(uptr, (int32_t)high, NULL, NULL) != SCPE_OK))) {
    ...
    for (j = 0, r = SCPE_OK; j < attcnt; j++) {
        if ((r == SCPE_OK) && (!dont_detach_attach)) {
            ...
            if ((!force_restore) && (!stat(attnames[j], &fstat)))
                if (fstat.st_mtime > rstat.st_mtime + 30) {
                    r = SCPE_INCOMP;
                    sim_printf(
                        "Error Attaching %s to %s - the restore state is %d seconds older than the attach file\n",
...
t_stat run_cmd(int32_t flag, const char *cptr)
{
        ...
        if (*cptr != 0) {                                          /* argument? */
            ...
            if (MATCH_CMD(gbuf, "UNTIL") != 0) {
                ...
                if ((tptr == gbuf) || (*tptr != 0) ||              /* error? */
                    (new_pcv > ((sim_PC->maxval > 0) ? sim_PC->maxval : (width_mask[sim_PC->width]))))
                    return SCPE_ARG;
                new_pc = true;
            }
        }
    ...
    else if ((flag == RU_STEP) || ((flag == RU_NEXT) && !sim_vm_is_subroutine_call)) { /* step */
        static bool not_implemented_message = false;
...
void run_cmd_message(const char *unechoed_cmdline, t_stat r)
{
...
void fprint_stopped_gen(FILE *st, t_stat v, REG *pc, DEVICE *dptr)
{
    ...
    if ((pc->flags & REG_VMAD) && sim_vm_fprint_addr) /* if reg wants VM-specific printer */
        ...
        fprint_val(st, pcval, pc->radix, pc->width,   /* otherwise, print as a numeric value */
...
t_stat exdep_cmd(int32_t flag, const char *cptr)
{
    ...
    for (gptr = gbuf, reason = SCPE_OK; (*gptr != 0) && (reason == SCPE_OK); gptr = tptr) {
        ...
        if (strncmp(gptr, "STATE", strlen("STATE")) == 0) {
            ...
            reason = exdep_reg_loop(ofile, sim_schrptr, flag, cptr, lowr, --highr, 0, 0xFFFFFFFF);
            if ((!sim_oline) && (sim_log && (ofile == stdout)))
                exdep_reg_loop(sim_log, sim_schrptr, EX_E, cptr, lowr, --highr, 0, 0xFFFFFFFF);
            continue;
        }
        ...
        if ((lowr = find_reg(gptr, &tptr, tdptr)) || /* local reg or */
            ...
            if ((*tptr == '-') || (*tptr == ':')) {
                ...
                if (*tptr == '[') {
                    if (lowr->depth <= 1)
                        return SCPE_ARG;
                    tptr = get_range(NULL, tptr + 1, &low, &high, 10, lowr->depth - 1, ']');
                    if (tptr == NULL)
                        return SCPE_ARG;
                }
            }
            if (*tptr && (*tptr++ != ','))
                return SCPE_ARG;
            reason = exdep_reg_loop(ofile, sim_schrptr, flag, cptr, lowr, highr, (uint32_t)low, (uint32_t)high);
            if ((flag & EX_E) && (!sim_oline) && (sim_log && (ofile == stdout)))
                exdep_reg_loop(sim_log, sim_schrptr, EX_E, cptr, lowr, highr, (uint32_t)low, (uint32_t)high);
            continue;
        }
        ...
        tptr = get_range(sim_dfdev, gptr, &low, &high, sim_dfdev->aradix,
                         (((sim_dfunit->capac == 0) || (flag == EX_E)) ? 0 : sim_dfunit->capac - sim_dfdev->aincr), 0);
        if (tptr == NULL)
            return (tstat ? tstat : SCPE_ARG);
        if (*tptr && (*tptr++ != ','))
            return SCPE_ARG;
        reason = exdep_addr_loop(ofile, sim_schaptr, flag, cptr, low, high, sim_dfdev, sim_dfunit);
        if ((flag & EX_E) && (!sim_oline) && (sim_log && (ofile == stdout)))
            exdep_addr_loop(sim_log, sim_schaptr, EX_E, cptr, low, high, sim_dfdev, sim_dfunit);
    } /* end for */
                      ...
                      uint32_t highs)
{
    ...
    for (rptr = lowr; rptr <= highr; rptr++) {
        if ((sim_switches & SIM_SW_HIDE) && (rptr->flags & REG_HIDDEN))
            continue;
        val = last_val = 0;
        limits = highs;
        if (highs == 0xFFFFFFFF)
...
t_stat ex_reg(FILE *ofile, t_value val, int32_t flag, REG *rptr, uint32_t idx)
{
    ...
    if ((rptr->flags & REG_VMAD) && sim_vm_fprint_addr)
        sim_vm_fprint_addr(ofile, sim_dflt_dev, (t_addr)val);
             ...
             (fprint_sym(ofile, (rptr->flags & REG_UFMASK) | rdx, sim_eval, NULL, sim_switches | SIM_SW_REG) > 0)) {
    ...
    if ((rptr->flags & REG_VMAD) && sim_vm_parse_addr) { /* address form? */
        ...
        if ((tptr == cptr) || (*tptr != 0) || (val > max))
            return SCPE_ARG;
               ...
               (parse_sym((const char *)cptr, (rptr->flags & REG_UFMASK) | rdx, NULL, &val, sim_switches | SIM_SW_REG) >
                SCPE_OK)) {
...
void put_rval_pcchk(REG *rptr, uint32_t idx, t_value val, bool pc_chk)
{
    ...
    if ((!(sim_switches & SWMASK('Z'))) && (rptr->flags & REG_DEPOSIT) && sim_vm_reg_update)
    ...
    if (rptr->size == sizeof(uint8_t))                   /* store the value */
        *((uint8_t *)ptr) =                              /*   using a size */
            (uint8_t)((*((uint8_t *)ptr) & mask) | val); /*     appropriate to */
                                                         /*       the size of */
    else if (rptr->size == sizeof(uint16_t))             /*         the underlying type */
        *((uint16_t *)ptr) = (uint16_t)((*((uint16_t *)ptr) & mask) | val);

    else if (rptr->size == sizeof(uint32_t))
        *((uint32_t *)ptr) = (uint32_t)((*((uint32_t *)ptr) & mask) | val);
...
t_stat get_aval(t_addr addr, DEVICE *dptr, UNIT *uptr)
{
    ...
    for (i = 0, j = addr; i < sim_emax; i++, j = j + dptr->aincr) {
        if (dptr->examine != NULL) {
            ...
            if (!(uptr->flags & UNIT_ATT))
                return SCPE_UNATT;
            if ((uptr->dynflags & UNIT_NO_FIO) || (uptr->fileref == NULL))
                return SCPE_NOFNC;
            ...
            if (uptr->flags & UNIT_BUF) {
                ...
                if ((feof(uptr->fileref)) && !(uptr->flags & UNIT_FIX)) {
                    reason = SCPE_EOF;
                    break;
...
char *read_line_p(const char *prompt, char *cptr, int32_t size, FILE *stream)
{
    ...
    for (tptr = cptr; tptr < (cptr + size); tptr++) {                            /* remove cr or nl */
        if ((*tptr == '\n') || (*tptr == '\r') || (tptr == (cptr + size - 1))) { /* str max length? */
            *tptr = 0;                                                           /* terminate */
            break;
        }
    }
...
const char *get_range(DEVICE *dptr, const char *cptr, t_addr *lo, t_addr *hi, uint32_t rdx, t_addr max, char term)
{
    const char *tptr;
...
t_stat sim_decode_quoted_string(const char *iptr, uint8_t *optr, uint32_t *osize)
{
    ...
    *osize = 0;
    if ((strlen(iptr) == 1) || (iptr[0] != iptr[strlen(iptr) - 1]) ||
            ...
            if ((rptr = find_reg(cptr, optr, dptr))) { /* found? */
                if (srptr) {                           /* ambig? err */
                    if (stat) {
                        if (sim_show_message) {
                            if (*stat == SCPE_OK)
                                sim_printf("Ambiguous register.  %s appears in devices %s and %s", cptr, (*gdptr)->name,
                                           dptr->name);
                            else
    ...
    *st = SCPE_OK;
    while (*cptr) {                                 /* loop through modifiers */
        ...
        if ((opt & CMD_OPT_SCH) &&                                /* if allowed, */
            ...
            sim_schaptr =
                get_asearch(gbuf, sim_dfdev->dradix, &sim_staba); /* populate memory version of the same expression */
            ...
            return svptr;                                         /* not rec, break out */
    }
    return cptr;
}
...
t_value strtotv(const char *inptr, const char **endptr, uint32_t radix)
{
    ...
    while (sim_isspace(*inptr)) /* bypass white space */
        inptr++;
    if (((radix == 0) || (radix == 16)) && ((!memcmp("0x", inptr, 2)) || (!memcmp("0X", inptr, 2)))) {
        radix = 16;
        inptr += 2;
    }
    if (((radix == 0) || (radix == 2)) && ((!memcmp("0b", inptr, 2)) || (!memcmp("0B", inptr, 2)))) {
        radix = 2;
        inptr += 2;
    }
...
t_stat sprint_val(char *buffer, t_value val, uint32_t radix, size_t width, uint32_t format)
{
...
t_stat fprint_val(FILE *stream, t_value val, uint32_t radix, uint32_t width, uint32_t format)
{
    char dbuf[MAX_WIDTH + 1];
...
t_stat sim_print_val(t_value val, uint32_t radix, uint32_t width, uint32_t format)
{
    char dbuf[MAX_WIDTH + 1];
    t_stat stat = SCPE_OK;
...
const char *sim_fmt_secs(double seconds)
{
    ...
    if (days > 0)
        snprintf(buf, sizeof(buf), "%s%.0f day%s %02.0f:%02.0f:%02.0f%s hour%s", sign, days, (days != 1) ? "s" : "",
                 hours, mins, secs, frac, (days == 1) ? "s" : "");
    else if (hours > 0)
...
const char *sim_fmt_numeric(double number)
{
    ...
    for (c = 0, p = buf; c < len; c++) {
        if ((c > 0) && (sim_isdigit(tmpbuf[c])) && (0 == ((len - c) % 3)))
            *(p++) = ',';
        *(p++) = tmpbuf[c];
    }
    *p = '\0';
...
t_stat sim_process_event(void)
{
    ...
    if (sim_interval < 0) {
        ...
        sim_interval = 0;
        UPDATE_SIM_TIME; /* update sim time */
        sim_debug(SIM_DBG_EVENT_NEG, &sim_scp_dev,
                  "Processing event for %s with sim_interval = %d, event time = %.0f\n", sim_uname(sim_clock_queue),
                  sim_interval_catchup, sim_gtime());
    ...
    do {
        ...
        if ((bare_reason != SCPE_OK) && /* Provide context for unexpected errors */
            (bare_reason >= SCPE_BASE) && (bare_reason != SCPE_EXPECT) && (bare_reason != SCPE_REMOTE) &&
            (bare_reason != SCPE_MTRLNT) && (bare_reason != SCPE_STOP) && (bare_reason != SCPE_STEP) &&
            (bare_reason != SCPE_RUNTIME) && (bare_reason != SCPE_EXIT)) {
 ...
 */
static void _sim_debug_write_flush(const char *buf, size_t len, bool flush)
{
    char *eol;
    ...
    while (NULL != (eol = strchr(debug_line_buf, '\n')) || flush) {
        ...
        size_t linesize = (eol != NULL) ? (size_t)(eol - debug_line_buf) + 1 : debug_line_offset;
        ...
        if ((0 != memcmp("DBG(", debug_line_buf, 4)) || (endprefix == NULL)) {
            ...
            const size_t compare_len =
                (eol != NULL) ? (size_t)(eol - endprefix + 1) : linesize - (size_t)(endprefix - debug_line_buf);
            ...
            if (debug_line_count == 0) {
                ...
                if (debug_line_buf_last_len == compare_len &&
                    0 == memcmp(&debug_line_buf[endprefix - debug_line_buf],
                                &debug_line_buf_last[debug_line_buf_last_endprefix_offset], compare_len)) {
                    ++debug_line_count;
...
static const char *sim_debug_prefix(uint32_t dbits, const DEVICE *dptr, const UNIT *uptr)
{
    ...
    if (sim_deb_switches & (SWMASK('T') | SWMASK('R') | SWMASK('A'))) {
        ...
        if (sim_deb_switches & SWMASK('T')) {
            ...
            if (now != NULL)
                snprintf(tim_t, sizeof(tim_t), "%02d:%02d:%02d.%03d ", now->tm_hour, now->tm_min, now->tm_sec,
                         (int)(time_now.tv_nsec / 1000000));
        }
        if (sim_deb_switches & SWMASK('A')) {
            snprintf(tim_t, sizeof(tim_t), "%" PRIdMAX ".%03d ", (intmax_t)time_now.tv_sec,
                     (int)(time_now.tv_nsec / 1000000));
        }
    }
    ...
    snprintf(debug_line_prefix, sizeof(debug_line_prefix), "DBG(%s%s%.0f%s)%s> %s %s: ", tim_t, tim_a, sim_gtime(),
             pc_s, is_simulator_thread() ? "" : "+", dptr->name, debug_type);
    return debug_line_prefix;
}

static void sim_fprint_bitfield_value(FILE *stream, BITF_FMT format, uint32_t value)
{
    switch (format) {
...
void sim_debug_bits_hdr(uint32_t dbits, DEVICE *dptr, const char *header, BITFIELD *bitdefs, uint32_t before,
...
void sim_debug_bits(uint32_t dbits, DEVICE *dptr, BITFIELD *bitdefs, uint32_t before, uint32_t after, int terminate)
{
...
t_stat sim_messagef(t_stat stat, const char *fmt, ...)
{
    ...
    while (1) { /* format passed string, args */
        ...
        formatted_len = vsnprintf(buf + prefix_len, bufsize - prefix_len, fmt, arglist);
        va_end(arglist);
...
void sim_data_trace(DEVICE *dptr, UNIT *uptr, const uint8_t *data, const char *position, size_t len, const char *txt,
                    uint32_t reason)
{
...
static MFILE *MOpen(void)
{
    return (MFILE *)calloc(1, sizeof(MFILE));
}

static void MFlush(MFILE *f)
{
    f->pos = 0;
}
...
static int FMwrite(FILE *fout, MFILE *fdata)
{
    ...
    for (i = 0; (dptr = devices[i]) != NULL; i++) {
        ...
        for (rptr = dptr->registers; (rptr != NULL) && (rptr->name != NULL); rptr++) {
            ...
            if (sim_switches & SWMASK('R')) /* Debug output */
                sim_printf("%5s:%-9.9s %s(rdx=%u, wd=%u, off=%u, dep=%u, strsz=%zu, objsz=%zu, elesz=%zu, rsz=%zu, %s "
                           "%s%s membytes=%zu, macro=%s)\n",
                           dptr->name, rptr->name, rptr->macro, rptr->radix, rptr->width, rptr->offset, rptr->depth,
                           rptr->stride, rptr->obj_size, rptr->size, rsz, rptr->desc ? rptr->desc : "",
...
} validations[] = {{{{ORDATAD(REG8, treg8, 8 * sizeof(treg8), "8 bit register")},
...
static DEVICE validate_test = {"TEST-REG", validate_units, NULL, NULL, 3, 16, 22, 4, 16, 16};

static t_stat test_register_validation(void)
{
    struct validation_test *v;
    DEVICE *v_devs[] = {&validate_test, NULL};
...
static struct parse_function_test {
    ...
    {"get_glyph", get_glyph, "AbcDe", {{0, "ABCDE", ""}}},
    {"get_glyph", get_glyph, "AbcDe", {{'c', "AB", "De"}, {'c', "DE", ""}}},
    {"get_glyph", get_glyph, "Ab cde", {{0, "AB", "cde"}, {0, "CDE", ""}}},
    {"get_glyph_nc", get_glyph_nc, "AbcDe", {{0, "AbcDe", ""}}},
    {"get_glyph_nc", get_glyph_nc, "AbcDe", {{'c', "Ab", "De"}, {'c', "De", ""}}},
    {"get_glyph_nc", get_glyph_nc, "Ab cde", {{0, "Ab", "cde"}, {0, "cde", ""}}},
    {"get_glyph_quoted", get_glyph_quoted, "AbcDe", {{0, "AbcDe", ""}}},
    {"get_glyph_quoted", get_glyph_quoted, "AbcDe", {{'c', "Ab", "De"}, {'c', "De", ""}}},
    {"get_glyph_quoted", get_glyph_quoted, "Abc De", {{0, "Abc", "De"}, {0, "De", ""}}},
    {"get_glyph_quoted", get_glyph_quoted, "\'AbcDe\'", {{0, "\'AbcDe\'", ""}}},
    {"get_glyph_quoted", get_glyph_quoted, "\'AbcDe\'", {{'c', "\'AbcDe\'", ""}}},
    {"get_glyph_quoted", get_glyph_quoted, "\'Abc De\'", {{0, "\'Abc De\'", ""}}},
    {"get_glyph_quoted", get_glyph_quoted, "\"AbcDe\"", {{0, "\"AbcDe\"", ""}}},
    {"get_glyph_quoted", get_glyph_quoted, "\"AbcDe\"", {{'c', "\"AbcDe\"", ""}}},
    {"get_glyph_quoted", get_glyph_quoted, "\"Abc De\"", {{0, "\"Abc De\"", ""}}},
    {"get_glyph_quoted", get_glyph_quoted, "\"Abc\" De", {{0, "\"Abc\"", "De"}, {0, "De", ""}}},
    {"get_glyph_quoted", get_glyph_quoted, "\'Abc\' De", {{'c', "\'Abc\'", "De"}, {'c', "De", ""}}},
    {"get_glyph_quoted", get_glyph_quoted, "\'Abc\' \"De\"", {{0, "\'Abc\'", "\"De\""}, {'c', "\"De\"", ""}}},
    {"get_glyph_quoted", get_glyph_quoted, "\'Ab\\c\' \"D\\e\"", {{0, "\'Ab\\c\'", "\"D\\e\""}, {'c', "\"D\\e\"", ""}}},
    {NULL}};
...
static t_stat test_scp_parsing(void)
{
    ...
    while (t->function_name) {
        ...
        while ((input != NULL) && (*input != '\0')) {
            ...
            if ((0 != strcmp(gbuf, d->expected_result)) || (remainder == NULL) ||
                (0 != strcmp(remainder, d->expected_remainder))) {
                if (0 != strcmp(gbuf, d->expected_result))
                    result = sim_messagef(SCPE_IERR,
                                          "function: %s (\"%s\", gbuf, %s); returned an unexpected result string: "
                                          "\"%s\" instead of \"%s\"\n",
                                          t->function_name, input, end_char_string, gbuf, d->expected_result);
                if (remainder == NULL)
                    result = sim_messagef(
                        SCPE_IERR,
                        "function: %s (\"%s\", gbuf, %s); returned a NULL pointer for a remnant instead of \"%s\"\n",
                        t->function_name, input, end_char_string, d->expected_result);
                else {
                    if (0 != strcmp(remainder, d->expected_remainder))
                        result = sim_messagef(
                            SCPE_IERR,
                            "function: %s (\"%s\", gbuf, %s); returned a remnant of \"%s\" instead of \"%s\"\n",
...
} parse_arg_function_tests[] = {{"1 2 3 4", {5, {"", "1", "2", "3", "4", NULL}}},
                                {"'1 2 3 4'", {2, {"", "1 2 3 4", NULL}}},
                                {"1 \"2\" 3 4", {5, {"", "1", "2", "3", "4", NULL}}},
                                {NULL}};
...
static t_stat test_scp_event_sequencing(void)
{
    ...
    sim_interval =
        -1; /* This should cover the remaining event on the last unit from above and leave the timer based one */
    r = sim_process_event();
    if (r != SCPE_OK)
        return sim_messagef(SCPE_IERR, "sim_process_event() unexpected result: %s\n", sim_error_text(r));
    for (i = 0, active = 0; i < dptr->numunits; i++) {
...
t_stat test_lib_cmd(int32_t flag, const char *cptr)
{
    ...
    for (i = 0; (dptr = sim_devices[i]) != NULL; i++) {
        t_stat tstat = SCPE_OK;
        bool was_disabled = ((dptr->flags & DEV_DIS) != 0);
        ...
        if (tstat != SCPE_OK) {
            stat = tstat;
            sim_printf("%s device tests returned: %d - %s\n", dptr->name, SCPE_BARE_STATUS(tstat),
                       sim_error_text(tstat));
            if (sim_ttisatty()) {
